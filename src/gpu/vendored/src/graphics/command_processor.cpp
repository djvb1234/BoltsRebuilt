...
#include "native/native_precise_wait.h"
#include "native/native_precise_wait_policy.h"
#include "native/native_wait_poll.h"

REXCVAR_DEFINE_BOOL(vsync, true, "GPU", "Enable vertical sync");

REXCVAR_DEFINE_BOOL(nb_native_wait_diagnostics, false, "nb",
                    "Measure actual empty command-ring waits and blocked WAIT_REG_MEM packets without changing wait behavior");

REXCVAR_DEFINE_BOOL(nb_native_precise_wait, false, "nb",
                    "Use a private high-resolution timer for existing WAIT_REG_MEM sleeps, retaining their requested duration and legacy fallback")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_early_wait_poll, false, "nb",
                    "Experiment with one 250us first retry of eligible WAIT_REG_MEM packets; subsequent waits retain the full duration")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_INT32(nb_native_wait_spin_us, 0, "nb",
                     "Re-read a blocked WAIT_REG_MEM predicate at yield cadence for up to this many microseconds per packet before the original sleeps resume; 0 keeps the original sleeps")
    .range(0, 20000)
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_wait_target_diagnostics, false, "nb",
                    "Record which register or memory word blocked WAIT_REG_MEM packets poll, how long they block and whether a vblank passed meanwhile")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

namespace {
// Monotonic nanoseconds for the optional WAIT_REG_MEM spin budget and target diagnostics.
uint64_t NativeWaitNowNs() {
...
REXCVAR_DEFINE_BOOL(nb_native_register_fastpath, true, "nb",
                    "Batch sequential type-0 register writes through the existing constant-range path and defer disabled diagnostic lookups")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
...
// nb: Phase 3 item 6 named counter. Counts one tile pass when a
// PM4_SET_BIN_SELECT* write takes effect while the binning state differs
// from the non-tiling defaults (bin_select_ / bin_mask_ both start at
// 0xFFFFFFFFull, command_processor.h:263-264) and the new selection actually
// enables a bin. UNSURE: this is an INFERRED heuristic - no SDK comment
// defines "tile pass"; the guest's tiling loop writes a new select per pass,
...
void CommandProcessor::WorkerThreadMain() {
  ...
  while (worker_running_) {
    ...
    if (write_ptr_index == 0xBAADF00D || read_ptr_index_ == write_ptr_index) {
      ...
      // event is too high.
      PrepareForWait();
      const bool measure_idle = REXCVAR_GET(nb_native_wait_diagnostics);
      auto& wait_stats = GetNativeCommandWaitStats();
      if (measure_idle) ++wait_stats.idle_periods;
      NativeCommandWaitTimer idle_timer(measure_idle, wait_stats.idle_ns);
      idle_timer.Start();
...
void CommandProcessor::WriteRegister(uint32_t index, uint32_t value) {
  ...
  if (REXCVAR_GET(nb_native_register_fastpath)) {
    // Register names are only diagnostic. Preserve live debug-level changes,
    // while avoiding the large metadata switch for every ordinary register write.
    auto* logger = ::rex::GetLoggerRaw(::rex::log::gpu());
    if (logger && logger->should_log(spdlog::level::debug) && !regs.GetRegisterInfo(index)) {
      REXGPU_DEBUG("GPU: Write to unknown register ({:04X} = {:08X})", index, value);
    }
  } else if (!regs.GetRegisterInfo(index)) {
    REXGPU_DEBUG("GPU: Write to unknown register ({:04X} = {:08X})", index, value);
  }
...
bool CommandProcessor::ExecutePacketType0(memory::RingBuffer* reader, uint32_t packet) {
  ...
  if (REXCVAR_GET(nb_native_register_fastpath) && !write_one_reg) {
    // Reuse SET_CONSTANT's ring-wrap handling and virtual range writer. The
    // D3D12 override batches only constant ranges with their existing dirty
    // notifications; scratch, gamma, coherency and other registers still use
    // individual WriteRegister calls in the base implementation.
...
bool CommandProcessor::ExecutePacketType3(memory::RingBuffer* reader, uint32_t packet) {
  ...
  switch (opcode) {
    ...
    case PM4_SET_BIN_SELECT_LO: {
      ...
      result = true;
      NbCountTilePassIfBinning(bin_select_, bin_mask_);
    } break;
...
bool CommandProcessor::ExecutePacketType3_WAIT_REG_MEM(memory::RingBuffer* reader, uint32_t packet,
  ...
  const bool measure_wait = REXCVAR_GET(nb_native_wait_diagnostics);
  auto& wait_stats = GetNativeCommandWaitStats();
  if (measure_wait) ++wait_stats.wait_reg_packets;
  NativeCommandWaitTimer wait_timer(measure_wait, wait_stats.wait_reg_ns);
  bool counted_block = false;
  nb::gpu::NativeEarlyWaitPoll early_poll;
  // Optional bounded re-read at yield cadence (nb_native_wait_spin_us), counted per packet.
  nb::gpu::NativeWaitSpinBudget spin_budget(
      uint64_t(std::max(0, REXCVAR_GET(nb_native_wait_spin_us))) * 1000);
  // Optional target diagnostic. The vblank counter is written by the GPU VSync
  // thread; like the existing EVENT_WRITE_SHD read, this only samples it.
  const bool record_target = REXCVAR_GET(nb_native_wait_target_diagnostics);
  const volatile uint32_t& vblank_counter = counter_;
  bool blocked = false, advance_seen = false;
  uint32_t first_value = 0, last_value = 0, vblank_at_block = 0;
  uint64_t block_start_ns = 0, advance_seen_ns = 0;
  ...
  do {
    ...
    if (is_memory) {
      value =
          *reinterpret_cast<volatile uint32_t*>(memory_->TranslatePhysical(poll_reg_addr & ~uint32_t(0x3)));
    ...
    last_value = value;
    const auto read = early_poll.ObservePredicate(matched);
    if (measure_wait && read.early_match) ++wait_stats.early_matches;
    if (!matched) {
      if (!blocked) {
        blocked = true;
        if (record_target) {
          first_value = value;
          vblank_at_block = vblank_counter;
          block_start_ns = NativeWaitNowNs();
        }
      }
      if (measure_wait) {
        wait_timer.Start();
        if (!counted_block) { ++wait_stats.blocked_packets; counted_block = true; }
      }
      // Wait.
      if (wait >= 0x100) {
        PrepareForWait();
        ...
        if (!vsync_enabled) {
          ...
          if (measure_wait) ++wait_stats.yield_calls;
          rex::thread::MaybeYield();
        } else if (spin_budget.ShouldSpin(NativeWaitNowNs())) {
          // The Xenos CP re-polls in microseconds; the original loop sleeps at least
          // 1 ms between reads. Re-read at the SDK's own yield cadence while this
          // packet's budget lasts. The predicate, endian and coherency handling
          // above are unchanged, and the original sleeps resume once it is spent.
          ++nb::gpu::GetNativeWaitSpinStats().yields;
          if (measure_wait) ++wait_stats.yield_calls;
          rex::thread::MaybeYield();
          if (record_target && !advance_seen && vblank_counter != vblank_at_block) {
            advance_seen = true;
            advance_seen_ns = NativeWaitNowNs();
          }
        } else {
          if (measure_wait) ++wait_stats.sleep_calls;
          const uint32_t requested_ms = wait / 0x100;
          NativeCommandWaitTimer sleep_timer(measure_wait, wait_stats.actual_sleep_ns);
          sleep_timer.Start();
          if (nb::gpu::NativeEarlyWaitPoll::ShouldWaitEarly(
                  read, wait, wait_info & 7, vsync_enabled,
                  REXCVAR_GET(nb_native_early_wait_poll))) {
            // One diagnostic earlier observation, not cycle-accurate emulation.
            // Keep the normal loop's fresh read, predicate and coherence hooks.
            if (measure_wait) ++wait_stats.early_polls;
            const bool shortened = nb::gpu::NativePreciseSleepNs(
                nb::gpu::NativeEarlyWaitPoll::kWaitNanoseconds);
            early_poll.DidShortWait(shortened);
            if (shortened) {
              if (measure_wait)
                wait_stats.requested_sleep_ns += nb::gpu::NativeEarlyWaitPoll::kWaitNanoseconds;
            } else {
              if (measure_wait) {
                ++wait_stats.early_fallbacks;
                wait_stats.requested_sleep_ns += uint64_t(requested_ms) * 1000000;
              }
              // Any failed short timer gets the original FULL Sleep, including
              // when the timer already consumed some time before failing.
              rex::thread::Sleep(std::chrono::milliseconds(requested_ms));
            }
          } else if (REXCVAR_GET(nb_native_precise_wait)) {
            if (measure_wait)
              wait_stats.requested_sleep_ns += uint64_t(requested_ms) * 1000000;
            if (measure_wait) ++wait_stats.precise_calls;
            if (!nb::gpu::NativePreciseSleep(requested_ms)) {
              if (measure_wait) ++wait_stats.precise_fallbacks;
              rex::thread::Sleep(std::chrono::milliseconds(requested_ms));
            }
          } else {
            if (measure_wait)
  ...
  if (spin_budget.started()) {
    auto& spin_stats = nb::gpu::GetNativeWaitSpinStats();
    ++spin_stats.spinning_packets;
    if (spin_budget.exhausted()) {
      ++spin_stats.budget_exhausted;
    } else {
      ++spin_stats.matched_while_spinning;
    }
  }
  if (record_target && blocked) {
    const uint64_t end_ns = NativeWaitNowNs();
    nb::gpu::GetNativeWaitTargetTable().Record(nb::gpu::NativeWaitSample{
        is_memory, poll_reg_addr, wait_info & 7, mask, ref, first_value, last_value,
        end_ns - block_start_ns, vblank_counter != vblank_at_block, advance_seen,
        advance_seen ? end_ns - advance_seen_ns : 0});
  }
