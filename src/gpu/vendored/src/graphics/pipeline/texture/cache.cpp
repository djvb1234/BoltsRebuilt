...
REXCVAR_DEFINE_BOOL(nb_native_texture_outdated_load_first, false, "GPU",
                    "Read the texture invalidation flag before consuming a pending notification")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

// nb: memo of the two guest-layout extents FindOrCreateTexture needs at a render scale above 1x
// (docs/ultrawide-uw16-texture-layout-memo.md). The layout is a pure function of ten key fields, which the
// memo key packs exactly, so equal packed keys have equal extents. IsRangeScaledResolved, the part that changes
// as resolves happen, still runs on every call.
REXCVAR_DEFINE_BOOL(nb_texture_layout_memo, false, "nb",
                    "Reuse the guest-layout extents of equal texture layouts when checking for scaled resolves");
REXCVAR_DEFINE_STRING(nb_texture_layout_memo_frames, "", "nb",
                      "Same-run test: use nb_texture_layout_memo only in frames first-last[,first-last...]");
REXCVAR_DEFINE_STRING(nb_texture_layout_memo_verify_frames, "", "nb",
                      "Diagnostic: in frames first-last, use the memo and also recompute every hit, counting "
                      "mismatches (extra CPU work, so never time inside it)");

// nb: memo of FindOrCreateTexture's scaled-resolve decision (docs/ultrawide-uw17-scaled-resolve-memo.md). The
// decision is a function of the two ranges it checks and of the scaled-resolve page bitmap. Every change to that
// bitmap bumps a generation counter under the global lock before the bits change, and a memoised decision is
// reused only for the same ranges while the generation is unchanged, so no lock or bitmap scan is needed then.
REXCVAR_DEFINE_BOOL(nb_scaled_resolve_memo, false, "nb",
                    "Reuse FindOrCreateTexture's scaled-resolve decision while the scaled-resolve pages are unchanged");
REXCVAR_DEFINE_STRING(nb_scaled_resolve_memo_frames, "", "nb",
                      "Same-run test: use nb_scaled_resolve_memo only in frames first-last[,first-last...]");
REXCVAR_DEFINE_STRING(nb_scaled_resolve_memo_verify_frames, "", "nb",
                      "Diagnostic: in frames first-last, use the decision memo and also recompute every hit, "
                      "counting mismatches (extra CPU work, so never time inside it)");

// NbCommandProcessor counts a frame at its swap; the frame being recorded is one past the completed count.
extern "C" uint64_t NbGetCompletedSwapCount();

namespace {
// RequestTextures and the logger run on the sole command-thread consumer.
// Memory-watch producers never access these experiment counters.
nb::gpu::NativeTextureOutdatedPollStats g_native_texture_outdated_poll_stats;

// "first-last[,first-last...]"; an empty list contains no frame. The caller evaluates it once per frame.
bool NbLayoutMemoFrameInWindows(const std::string& windows, uint64_t frame) {
...
using NbLayoutBaseExtent =
    decltype(std::declval<rex::graphics::texture_util::TextureGuestLayout>().base.level_data_extent_bytes);
using NbLayoutMipsExtent =
    decltype(std::declval<rex::graphics::texture_util::TextureGuestLayout>().mips_total_extent_bytes);

struct NbLayoutMemoEntry {
  uint64_t key = 0;  // 0 is empty: every packed key has bit 63 set
  NbLayoutBaseExtent base_extent{};
  NbLayoutMipsExtent mips_extent{};
};
constexpr uint32_t kNbLayoutMemoBits = 10;
// Only the command thread calls FindOrCreateTexture, and there is one texture cache per process.
NbLayoutMemoEntry g_nb_layout_memo[size_t(1) << kNbLayoutMemoBits];
struct NbLayoutMemoStats {
  uint64_t hits = 0, misses = 0, verified = 0, mismatches = 0;
} g_nb_layout_memo_stats;

// Bumped under the global critical region, before the bits change, whenever MarkRangeAsResolved or the global
// watch callback changes a scaled-resolve page bit, and when the bitmap is cleared. Guest threads write it from
// the watch callback; the command thread reads it without the lock.
std::atomic<uint64_t> g_nb_scaled_resolve_generation{0};
struct NbScaledDecisionEntry {
  uint64_t extents = 0;  // base extent | mips extent << 32
  uint64_t pages = 0;    // base page | mip page << 17
  uint64_t generation = 0;
  bool valid = false;
  bool scaled = false;
};
constexpr uint32_t kNbScaledDecisionBits = 10;
// Only the command thread calls FindOrCreateTexture.
NbScaledDecisionEntry g_nb_scaled_decision[size_t(1) << kNbScaledDecisionBits];
struct NbScaledDecisionStats {
  uint64_t hits = 0, misses = 0, verified = 0, mismatches = 0, raced = 0;
} g_nb_scaled_decision_stats;
}  // namespace

const nb::gpu::NativeTextureOutdatedPollStats&
nb::gpu::GetNativeTextureOutdatedPollStats() noexcept {
  return g_native_texture_outdated_poll_stats;
}
...
TextureCache::TextureCache(const RegisterFile& register_file, SharedMemory& shared_memory,
  ...
  if (draw_resolution_scale_x > 1 || draw_resolution_scale_y > 1) {
    ...
    g_nb_scaled_resolve_generation.fetch_add(1, std::memory_order_acq_rel);
...
bool TextureCache::GetConfigDrawResolutionScale(uint32_t& x_out, uint32_t& y_out) {
  ...
  // nb: nb_ultrawide's render scale (docs/ultrawide.md, src/gpu/vendored/UPSTREAM.md). It applies only while
  // none of resolution_scale, draw_resolution_scale_x or draw_resolution_scale_y was set at all; any explicit
  // value, 1 included, wins. It writes no cvar, so F4 "Save to config" cannot carry it into 16:9 play.
  const auto explicitly_set = [](const char* name) {
    return rex::cvar::GetFlagSource(name) != rex::cvar::Source::kDefault;
  };
  if (!explicitly_set("resolution_scale") && !explicitly_set("draw_resolution_scale_x") &&
      !explicitly_set("draw_resolution_scale_y")) {
    uint32_t ultrawide_x = 1, ultrawide_y = 1;
    if (nb::gpu::UltrawideDefaultScale(&ultrawide_x, &ultrawide_y)) {
      config_x = std::max(uint32_t(1), ultrawide_x);
      config_y = std::max(uint32_t(1), ultrawide_y);
    }
  }
...
void TextureCache::MarkRangeAsResolved(uint32_t start_unscaled, uint32_t length_unscaled) {
  ...
  if (IsDrawResolutionScaled()) {
    ...
    for (uint32_t i = block_first; i <= block_last; ++i) {
      ...
      // nb: a newly set bit invalidates memoised scaled-resolve decisions; re-marking set pages does not.
      if ((scaled_resolve_pages_[i] & add_bits) != add_bits) {
        g_nb_scaled_resolve_generation.fetch_add(1, std::memory_order_acq_rel);
      }
...
void TextureCache::RequestTextures(uint32_t used_texture_mask) {
  ...
  if (nb::gpu::NativePollTextureOutdated(
          texture_became_outdated_, REXCVAR_GET(nb_native_texture_outdated_load_first),
          g_native_texture_outdated_poll_stats)) {
...
TextureCache::Texture* TextureCache::FindOrCreateTexture(TextureKey key) {
  ...
  if (IsDrawResolutionScaled() && key.tiled && IsScaledResolveSupportedForFormat(key)) {
    // nb: the two extents come from the layout memo when it is on (NbLayoutMemoEntry). Its window cvars are
    // evaluated once per frame; this runs whenever a texture binding changes.
    static uint64_t nb_memo_frame = UINT64_MAX;
    static bool nb_memo_on = false;
    static bool nb_memo_verify = false;
    static bool nb_memo_logged_on = false;
    static bool nb_decision_on = false;
    static bool nb_decision_verify = false;
    static bool nb_decision_logged_on = false;
    const uint64_t nb_frame = NbGetCompletedSwapCount() + 1;
    if (nb_frame != nb_memo_frame) {
      nb_memo_frame = nb_frame;
      nb_memo_verify = NbLayoutMemoFrameInWindows(REXCVAR_GET(nb_texture_layout_memo_verify_frames), nb_frame);
      nb_memo_on = REXCVAR_GET(nb_texture_layout_memo) || nb_memo_verify ||
                   NbLayoutMemoFrameInWindows(REXCVAR_GET(nb_texture_layout_memo_frames), nb_frame);
      nb_decision_verify =
          NbLayoutMemoFrameInWindows(REXCVAR_GET(nb_scaled_resolve_memo_verify_frames), nb_frame);
      nb_decision_on = REXCVAR_GET(nb_scaled_resolve_memo) || nb_decision_verify ||
                       NbLayoutMemoFrameInWindows(REXCVAR_GET(nb_scaled_resolve_memo_frames), nb_frame);
      if (nb_decision_on != nb_decision_logged_on) {
        nb_decision_logged_on = nb_decision_on;
        REXGPU_INFO("nb: scaled-resolve decision memo {} from frame {} (so far {} hits, {} misses, {} verified, "
                    "{} mismatches, {} raced; generation {})",
                    nb_decision_on ? "on" : "off", nb_frame, g_nb_scaled_decision_stats.hits,
                    g_nb_scaled_decision_stats.misses, g_nb_scaled_decision_stats.verified,
                    g_nb_scaled_decision_stats.mismatches, g_nb_scaled_decision_stats.raced,
                    g_nb_scaled_resolve_generation.load(std::memory_order_acquire));
      }
      if (nb_memo_on != nb_memo_logged_on) {
        nb_memo_logged_on = nb_memo_on;
        REXGPU_INFO("nb: texture layout memo {} from frame {} (so far {} hits, {} misses, {} verified, {} "
                    "mismatches)",
                    nb_memo_on ? "on" : "off", nb_frame, g_nb_layout_memo_stats.hits,
                    g_nb_layout_memo_stats.misses, g_nb_layout_memo_stats.verified,
                    g_nb_layout_memo_stats.mismatches);
      }
    }
    NbLayoutBaseExtent base_extent;
    NbLayoutMipsExtent mips_extent;
    if (nb_memo_on) {
      // Exactly the fields GetGuestLayout reads (pipeline/texture/cache.h), 60 bits, plus bit 63 so no key is 0.
      const uint64_t packed =
          uint64_t(key.dimension) | (uint64_t(key.width_minus_1) << 2) | (uint64_t(key.height_minus_1) << 15) |
          (uint64_t(key.tiled) << 28) | (uint64_t(key.packed_mips) << 29) |
          (uint64_t(key.depth_or_array_size_minus_1) << 30) | (uint64_t(key.pitch) << 40) |
          (uint64_t(key.mip_max_level) << 49) | (uint64_t(key.format) << 53) |
          (uint64_t(key.base_page != 0) << 59) | (uint64_t(1) << 63);
      NbLayoutMemoEntry& entry =
          g_nb_layout_memo[(packed * UINT64_C(0x9E3779B97F4A7C15)) >> (64 - kNbLayoutMemoBits)];
      if (entry.key == packed) {
        ++g_nb_layout_memo_stats.hits;
        base_extent = entry.base_extent;
        mips_extent = entry.mips_extent;
        if (nb_memo_verify) {
          const texture_util::TextureGuestLayout layout = key.GetGuestLayout();
          ++g_nb_layout_memo_stats.verified;
          if (layout.base.level_data_extent_bytes != base_extent ||
              layout.mips_total_extent_bytes != mips_extent) {
            ++g_nb_layout_memo_stats.mismatches;
            base_extent = layout.base.level_data_extent_bytes;
            mips_extent = layout.mips_total_extent_bytes;
          }
        }
      } else {
        ++g_nb_layout_memo_stats.misses;
        const texture_util::TextureGuestLayout layout = key.GetGuestLayout();
        base_extent = layout.base.level_data_extent_bytes;
        mips_extent = layout.mips_total_extent_bytes;
        entry.key = packed;
        entry.base_extent = base_extent;
        entry.mips_extent = mips_extent;
      }
    } else {
      const texture_util::TextureGuestLayout layout = key.GetGuestLayout();
      base_extent = layout.base.level_data_extent_bytes;
    ...
    auto nb_scaled_now = [&]() {
      return (base_extent && IsRangeScaledResolved(key.base_page << 12, base_extent)) ||
             (mips_extent && IsRangeScaledResolved(key.mip_page << 12, mips_extent));
    };
    bool nb_scaled;
    if (nb_decision_on) {
      // nb: the decision memo (NbScaledDecisionEntry), keyed on exactly the two ranges IsRangeScaledResolved
      // checks. The generation is read before computing, so a change during the computation forces a recompute
      // next time.
      const uint64_t nb_extents = uint64_t(base_extent) | (uint64_t(mips_extent) << 32);
      const uint64_t nb_pages = uint64_t(key.base_page) | (uint64_t(key.mip_page) << 17);
      NbScaledDecisionEntry& decision =
          g_nb_scaled_decision[((nb_extents * UINT64_C(0x9E3779B97F4A7C15)) ^
                                (nb_pages * UINT64_C(0xC2B2AE3D27D4EB4F))) >>
                               (64 - kNbScaledDecisionBits)];
      const uint64_t generation = g_nb_scaled_resolve_generation.load(std::memory_order_acquire);
      if (decision.valid && decision.extents == nb_extents && decision.pages == nb_pages &&
          decision.generation == generation) {
        ++g_nb_scaled_decision_stats.hits;
        nb_scaled = decision.scaled;
        if (nb_decision_verify) {
          const bool fresh = nb_scaled_now();
          ++g_nb_scaled_decision_stats.verified;
          if (fresh != nb_scaled) {
            // A difference only counts as a mismatch if no bitmap change happened in between.
            if (g_nb_scaled_resolve_generation.load(std::memory_order_acquire) == generation) {
              ++g_nb_scaled_decision_stats.mismatches;
            } else {
              ++g_nb_scaled_decision_stats.raced;
            }
            nb_scaled = fresh;
          }
        }
      } else {
        ++g_nb_scaled_decision_stats.misses;
        nb_scaled = nb_scaled_now();
        decision.extents = nb_extents;
        decision.pages = nb_pages;
        decision.generation = generation;
        decision.valid = true;
        decision.scaled = nb_scaled;
      }
    } else {
      nb_scaled = nb_scaled_now();
    }
...
void TextureCache::ScaledResolveGlobalWatchCallback(
  ...
  for (uint32_t i = resolve_l2_block_first; i <= resolve_l2_block_last; ++i) {
    ...
    while (rex::bit_scan_forward(resolve_l2_block, &resolve_block_relative_index)) {
      ...
      // nb: a cleared bit invalidates memoised scaled-resolve decisions (the global lock is held here).
