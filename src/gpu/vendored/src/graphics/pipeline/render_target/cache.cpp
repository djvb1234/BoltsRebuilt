...
// nb: Phase 3 step 2 preparation (digest section 2 item 4). Path of a
// JSON-lines file that records every copying EDRAM ownership transfer whose
// tile range intersects [nb_edram_transfer_log_first_tile,
// nb_edram_transfer_log_last_tile]. Empty = off. The default window is tiles
// 1536..2047 (the upper half of EDRAM, where the glow chain surfaces live),
// so the glow black-flash root cause can be proven or refuted before the
// step-2 allowlist filter itself is built.
REXCVAR_DEFINE_STRING(nb_edram_transfer_log, "", "nb",
                      "JSON-lines log path for EDRAM ownership transfers in the tile window");
REXCVAR_DEFINE_INT32(nb_edram_transfer_log_first_tile, 1536, "nb",
                     "First EDRAM tile of the transfer log window (inclusive)");
...
bool RenderTargetCache::Update(bool is_rasterization_done,
  ...
  // nb: the render scale lets the vertex estimate round for host rows (util/draw_extent_estimator.cpp, F2).
      ...
      std::min(GetRenderTargetHeight(pitch_tiles_at_32bpp, msaa_samples),
               draw_extent_estimator_.EstimateMaxY(
                   interlock_barrier_only
                       ...
                       : true,
                   vertex_shader, draw_resolution_scale_y()));
...
// nb: Phase 3 step 2 preparation (digest section 2 item 4). Logs one copying
// ownership transfer as a JSON line when its [start_tiles, end_tiles) range
// intersects the configured window and nb_edram_transfer_log names a file.
// Direction is encoded as src -> dst: the previous owner render target the
// pixels are copied from, and the new owner they are copied to. Called from
// the command processor thread only (from ChangeOwnership below).
// Plain-data view of a RenderTargetKey: the key union is a protected member of
// RenderTargetCache (cache.h:183), so file-scope helpers cannot name it.
struct NbEdramSurface {
  uint32_t base_tiles;
...
void NbLogEdramTransfer(const char* operation, uint32_t start_tiles, uint32_t end_tiles,
                        const NbEdramSurface& source_key, const NbEdramSurface& dest_key) {
  const std::string& path = REXCVAR_GET(nb_edram_transfer_log);
  if (path.empty()) {
    return;
  }
  const int32_t first_tile = REXCVAR_GET(nb_edram_transfer_log_first_tile);
  const int32_t last_tile = REXCVAR_GET(nb_edram_transfer_log_last_tile);
  if (static_cast<int32_t>(start_tiles) > last_tile ||
      static_cast<int32_t>(end_tiles) <= first_tile) {
    return;
  }
  // Lazily opened; reopened if the cvar path changes (the cvar is
  // hot-reloadable). Never closed, same lifetime as the process.
  ...
  if (open_path != path) {
    ...
    if (file == nullptr) {
      REXGPU_ERROR("nb_edram_transfer_log: failed to open '{}'", path);
      return;
    }
  }
  std::fprintf(file,
               "{\"type\":\"edram_transfer\",\"op\":\"%s\",\"start_tiles\":%u,\"end_tiles\":%u,"
               "\"src\":{\"base\":%u,\"pitch_tiles\":%u,\"msaa\":%u,\"format\":\"%s\",\"is_depth\":%u},"
               "\"dst\":{\"base\":%u,\"pitch_tiles\":%u,\"msaa\":%u,\"format\":\"%s\",\"is_depth\":%u}}\n",
               operation, start_tiles, end_tiles, source_key.base_tiles, source_key.pitch_tiles,
               source_key.msaa, source_key.format, source_key.is_depth, dest_key.base_tiles,
               dest_key.pitch_tiles, dest_key.msaa, dest_key.format, dest_key.is_depth);
  // Flush per line: the flash being chased is intermittent, and a buffered
  // log would lose the tail on a crash.
  std::fflush(file);
}

// nb: Phase 3 item 6 transfer counters plus the step-2 log, shared by the
// append and extend sites in ChangeOwnership below.
void NbRecordEdramTransfer(const char* operation, uint32_t start_tiles, uint32_t end_tiles,
                           const NbEdramSurface& source_key, const NbEdramSurface& dest_key) {
  ++GetPhase3Counters().transfer_count;
  GetPhase3Counters().transfer_tiles += end_tiles - start_tiles;
  NbLogEdramTransfer(operation, start_tiles, end_tiles, source_key, dest_key);
}
...
void RenderTargetCache::ChangeOwnership(RenderTargetKey dest, uint32_t start_tiles_base_relative,
  ...
  // nb: plain-data view for the file-scope transfer log helpers (RenderTargetKey is
  // protected, so the conversion has to happen inside the class).
  auto nb_surface = [](RenderTargetKey key) {
    return NbEdramSurface{key.base_tiles, key.GetPitchTiles(),
                          uint32_t(1) << uint32_t(key.msaa_samples), key.GetFormatName(),
                          uint32_t(key.is_depth)};
  };
  bool host_depth_encoding_different = dest.is_depth && GetPath() == Path::kHostRenderTargets &&
                                       IsHostDepthEncodingDifferent(dest.GetDepthFormat());
        ...
        if (!transfer_source.IsEmpty() && transfer_source != dest) {
          ...
          if (!resolve_clear_cutout ||
            ...
            if (!transfers_append_out->empty() &&
              ...
              // nb: only the newly covered tiles [it->first,
              // transfer_end_tiles) are counted and logged; the earlier part
              // of the transfer was recorded when it was appended.
              NbRecordEdramTransfer("extend", it->first, transfer_end_tiles, nb_surface(transfer_source),
                                    nb_surface(dest));
            } else {
                ...
                if (transfer_host_depth_source.IsEmpty() ||
                  ...
                  transfers_append_out->emplace_back(
                      ...
                      transfer_host_depth_source_rt_it != render_targets_.end()
                          ? transfer_host_depth_source_rt_it->second
                          : nullptr);
                  NbRecordEdramTransfer("append", it->first, transfer_end_tiles,
                                        nb_surface(transfer_source), nb_surface(dest));
                }
              }
            }
          }
