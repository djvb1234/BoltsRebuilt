...
REXCVAR_DEFINE_BOOL(nb_native_deferred_storage, false, "nb",
                   "Reuse constructed deferred-command storage between submissions while retaining all owned argument snapshots");

namespace rex::graphics::d3d12 {

namespace {
thread_local NativeDeferredStorageStats native_deferred_storage_stats;
}
const NativeDeferredStorageStats& GetNativeDeferredStorageStats() {
  return native_deferred_storage_stats;
}
...
DeferredCommandList::DeferredCommandList(DeferredCommandList&& other) noexcept
    : nb_budget_probe_(std::exchange(other.nb_budget_probe_, nullptr)),
      command_processor_(other.command_processor_),
      command_stream_(std::move(other.command_stream_)),
      command_stream_used_(std::exchange(other.command_stream_used_, 0)),
      reuse_command_storage_(other.reuse_command_storage_),
      pipeline_handles_resolved_for_replay_(
          std::exchange(other.pipeline_handles_resolved_for_replay_, false)) {}

void DeferredCommandList::RecordNativeBudgetOperation(uint32_t op, uint64_t copy_bytes) {
  nb_budget_probe_->Operation(*this, op, copy_bytes);
}

void DeferredCommandList::Reset() {
  reuse_command_storage_ = REXCVAR_GET(nb_native_deferred_storage);
  if (reuse_command_storage_) {
    ++native_deferred_storage_stats.enabled_resets;
  } else {
    command_stream_.clear();
  }
  command_stream_used_ = 0;
  pipeline_handles_resolved_for_replay_ = false;
}

void DeferredCommandList::SwapStorage(DeferredCommandList& other) noexcept {
  assert_true(&command_processor_ == &other.command_processor_);
  command_stream_.swap(other.command_stream_);
  std::swap(command_stream_used_, other.command_stream_used_);
  std::swap(reuse_command_storage_, other.reuse_command_storage_);
  std::swap(pipeline_handles_resolved_for_replay_, other.pipeline_handles_resolved_for_replay_);
}

bool DeferredCommandList::IsReadyForReplay() const noexcept {
  if (command_stream_used_ > command_stream_.size()) return false;
  size_t offset = 0;
  while (offset != command_stream_used_) {
    ...
    if (header.command == Command::kSetPipelineStateHandle) {
      if (header.arguments_size_elements !=
          (sizeof(void*) + sizeof(uintmax_t) - 1) / sizeof(uintmax_t)) return false;
      void* handle;
      std::memcpy(&handle, command_stream_.data() + offset, sizeof(handle));
      if (!handle || !command_processor_.GetD3D12PipelineByHandle(handle)) return false;
    }
    offset += header.arguments_size_elements;
  }
  return true;
}

bool DeferredCommandList::ResolvePipelineHandlesForReplay() noexcept {
  static_assert(sizeof(void*) == sizeof(ID3D12PipelineState*));
  pipeline_handles_resolved_for_replay_ = false;
  if (command_stream_used_ > command_stream_.size()) return false;
  // Validate each command before resolving it. This stream is owned and written
  // by this class. A malformed stream stays unmarked and must never be replayed;
  // earlier resolutions need no rollback because that failure is terminal.
  size_t offset = 0;
  while (offset != command_stream_used_) {
    if (command_stream_used_ - offset < kCommandHeaderSizeElements) return false;
    ...
    if (header.arguments_size_elements > command_stream_used_ - offset) return false;
    if (header.command == Command::kSetPipelineStateHandle &&
        header.arguments_size_elements != (sizeof(void*) + sizeof(uintmax_t) - 1) / sizeof(uintmax_t)) {
      return false;
    }
    if (header.command == Command::kSetPipelineStateHandle) {
      void* handle;
      std::memcpy(&handle, command_stream_.data() + offset, sizeof(handle));
      ID3D12PipelineState* pipeline = command_processor_.GetD3D12PipelineByHandle(handle);
      if (!pipeline) ++GetPhase3Counters().execute_time_null_handle_count;
      std::memcpy(command_stream_.data() + offset, &pipeline, sizeof(pipeline));
      header.command = Command::kD3DSetPipelineState;
    }
    offset += header.arguments_size_elements;
  }
  pipeline_handles_resolved_for_replay_ = true;
  return true;
}
...
void DeferredCommandList::Execute(ID3D12GraphicsCommandList* command_list,
  ...
  size_t stream_remaining = command_stream_used_;
  ReplayState local_state;
  auto& current_pipeline_state =
      (replay_state ? *replay_state : local_state).current_pipeline_state;
  while (stream_remaining != 0) {
    ...
    switch (header.command) {
      ...
      case Command::kD3DSetNativeGraphicsConstantBufferViews: {
        const auto& args =
            *reinterpret_cast<const SetNativeGraphicsConstantBufferViewsArguments*>(stream);
        command_list->SetGraphicsRootConstantBufferView(0, args.root0);
        command_list->SetGraphicsRootConstantBufferView(1, args.vertex);
        if (args.write_pixel) {
      ...
      case Command::kSetPipelineStateHandle: {
        ...
        if (current_pipeline_state) {
          ...
          // nb: Phase 3 step 6 (digest section 2 item 10). The handle still
          // resolves to null at Execute time. PipelineCache::EndSubmission
          // has already awaited all queued pipeline creations before this
          // list executes, so this means creation failed. The null current
          // pipeline suppresses subsequent draws/dispatches until another PSO
          // command succeeds. Count this on the CP thread (or during CP-side
          // ResolvePipelineHandlesForReplay before worker submission).
          ++GetPhase3Counters().execute_time_null_handle_count;
        }
      } break;
...
void* DeferredCommandList::WriteCommand(Command command, size_t arguments_size_bytes) {
  const size_t arguments_size_elements = arguments_size_bytes / sizeof(uintmax_t) +
      (arguments_size_bytes % sizeof(uintmax_t) != 0);
  const size_t offset = command_stream_used_;
  const size_t max_elements = command_stream_.max_size();
  if (arguments_size_elements > std::numeric_limits<uint32_t>::max() ||
      offset > max_elements || kCommandHeaderSizeElements > max_elements - offset ||
      arguments_size_elements > max_elements - offset - kCommandHeaderSizeElements) {
    throw std::length_error("deferred command exceeds stream bounds");
  }
  const size_t required = offset + kCommandHeaderSizeElements + arguments_size_elements;
  const size_t previous_size = command_stream_.size();
  if (required > previous_size) command_stream_.resize(required);
  // Publish the logical extent only after allocation succeeds. Keeping live
  // uintmax_t objects avoids both value-initializing them again after Reset
  // and writing into unconstructed vector capacity. Each command writer still
  // supplies every active field before returning; padding is never replayed.
  command_stream_used_ = required;
  if (reuse_command_storage_) {
    ++native_deferred_storage_stats.commands;
    native_deferred_storage_stats.reused_bytes +=
        (std::min(required, previous_size) - offset) * sizeof(uintmax_t);
    if (required > previous_size) {
      native_deferred_storage_stats.initialized_bytes +=
          (required - previous_size) * sizeof(uintmax_t);
    }
  }
