...
#include "native/native_shader_hash.h"
#include "native/native_shader_hash_policy.h"

REXCVAR_DEFINE_BOOL(d3d12_dxbc_disasm, false, "GPU/D3D12", "Dump DXBC disassembly");

REXCVAR_DEFINE_BOOL(nb_shader_load_memo, false, "nb",
                    "Reuse repeated shader loads only after comparing every current microcode byte; applies to native and emulated draws")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_avx2_shader_hash, false, "nb",
                    "Use exact XXH3 AVX2 shader hashing only on supported CPU/OS configurations")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
...
void PipelineCache::InitializeShaderStorage(const std::filesystem::path& cache_root,
  ...
  // Failed stored translations can delete their Shader after the preload
  // workers join. No pointer from a previous runtime load may survive that.
  shader_load_memo_.Reset();
  ShutdownShaderStorage();
...
D3D12Shader* PipelineCache::LoadShader(xenos::ShaderType shader_type, const uint32_t* host_address,
  ...
  const bool memo_enabled = REXCVAR_GET(nb_shader_load_memo);
  if (memo_enabled) {
    if (const uintptr_t cached = shader_load_memo_.Find(host_address, uint32_t(shader_type), dword_count)) {
      return reinterpret_cast<D3D12Shader*>(cached);
    }
  }
  // Hash the input memory and lookup the shader.
  const size_t byte_count = size_t(dword_count) * sizeof(uint32_t);
  const bool use_avx2_hash =
      byte_count >= nb::gpu::kNativeShaderHashLongMinimum &&
      REXCVAR_GET(nb_native_avx2_shader_hash);
  const uint64_t data_hash = use_avx2_hash
      ? nb::gpu::NativeShaderHash(host_address, byte_count, true)
      : XXH3_64bits(host_address, byte_count);
  D3D12Shader* shader = LoadShader(shader_type, host_address, dword_count,
                                  data_hash);
  if (memo_enabled && shader->type() == shader_type && shader->ucode_dword_count() == dword_count) {
    shader_load_memo_.Publish(host_address, uint32_t(shader_type), dword_count,
                              shader->ucode_dwords(), reinterpret_cast<uintptr_t>(shader));
  }
  return shader;
}
...
bool PipelineCache::PrepareShaderTranslations(
  ...
  // nb: Phase 3 step 6 - translation is always synchronous on the command processor
  // thread; only D3D12 pipeline creation stays asynchronous (use_async below).
...
bool PipelineCache::ConfigurePipeline(
  ...
  if (!PrepareShaderTranslations(vertex_shader, pixel_shader)) {
    return false;
  }
  ...
  if (!GetCurrentStateDescription(
          ...
          /* for_placeholder: shaders are translated above, so build the real root
             signature now (nb Phase 3 step 6) */ false)) {
    return false;
  }
