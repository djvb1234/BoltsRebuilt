...
// nb: Phase 3 step 1 (plan revision 2, digest section 2 item 12). Force the
// host render target (RTV) path by default. The pixel shader interlock (ROV)
// path stays compiled and stays selectable at runtime with
// render_target_path_d3d12 = "rov"; the vendor auto-default in Initialize
// below (Intel -> PSI, everything else -> RTV, lines ~181-199 upstream) is
// only reached when this cvar is explicitly set back to "". Step 1 is
// bookkeeping, not a de-emulation win: RTV is already the auto-default on
// every non-Intel GPU, so no performance change is expected from this.
REXCVAR_DEFINE_STRING(render_target_path_d3d12, "rtv", "GPU/D3D12",
...
// nb: diagnostic frame windows for measuring what EDRAM ownership transfers cost on the GPU in one run
// (docs/ultrawide.md, Performance; src/gpu/vendored/UPSTREAM.md). The value is "first-last" in the frame
// trace's numbering, and empty turns the window off. THE OUTPUT IS WRONG inside a window; never use these
// for play.
REXCVAR_DEFINE_STRING(nb_diag_skip_transfer_stencil_frames, "", "nb",
                      "Diagnostic, wrong output: skip the stencil clear and the eight stencil-bit passes of "
                      "depth transfers in frames first-last");
REXCVAR_DEFINE_STRING(nb_diag_skip_transfers_frames, "", "nb",
                      "Diagnostic, wrong output: skip every EDRAM ownership transfer draw in frames first-last");

// nb: per-pixel stencil-bit passes for multisampled depth transfers (docs/ultrawide-uw15-stencil-coverage.md).
// Without a pixel-shader stencil reference, a depth transfer rebuilds stencil in eight passes, one per bit.
// Upstream runs them per sample and discards the samples whose bit is clear. This variant runs one
// invocation per pixel, repeats the per-sample addressing for each of the destination's samples, and writes
// the samples whose bit is set to SV_Coverage, so the same samples receive the bit.
REXCVAR_DEFINE_BOOL(nb_transfer_stencil_coverage, false, "nb",
                    "Rebuild the stencil bits of multisampled depth transfers at pixel rate with SV_Coverage "
                    "instead of per sample");
REXCVAR_DEFINE_STRING(nb_transfer_stencil_coverage_frames, "", "nb",
                      "Same-run test: use nb_transfer_stencil_coverage only in frames first-last[,first-last...]");
REXCVAR_DEFINE_STRING(nb_transfer_stencil_coverage_verify_frames, "", "nb",
...
extern "C" uint64_t NbGetCompletedSwapCount();

namespace {

// "first-last[,first-last...]"; an empty list contains no frame.
bool NbFrameInWindows(const std::string& windows) {
  if (windows.empty()) return false;
  const unsigned long long frame = NbGetCompletedSwapCount() + 1;
...
// nb: sample-exact check of the per-pixel stencil-bit passes (nb_transfer_stencil_coverage_verify_frames).
// After a checked draw's eight variant passes, occlusion queries count per bit:
// - A, the samples the SDK's per-sample passes would set (its shader, stencil ALWAYS);
// - B, those of them whose destination bit is now set (its shader, stencil EQUAL on the bit);
// - C, every sample of the draw's rectangles whose bit is set (no pixel shader, EQUAL).
// The SDK cleared the stencil to 0 in those rectangles before any transfer draw, so A == B == C for all eight
// bits means the variant set exactly the SDK's bits; A - B samples would be missing and C - B extra. The check
// writes nothing. Draws recorded while a guest occlusion query is open are skipped, so that query never nests
// these or counts their samples. One check per launch; the render target cache is a single instance.
struct NbStencilVerifyDraw {
  uint32_t first_query;
  uint32_t shader_key;
  uint32_t vertex_count;
};
struct NbStencilVerifyState {
  static constexpr uint32_t kQueriesPerDraw = 24;
  static constexpr uint32_t kMaxDraws = 1024;
  // At most this many draws per shader key, so a long window reaches rare keys before the check is full.
  static constexpr uint32_t kMaxDrawsPerKey = 96;
  ID3D12QueryHeap* query_heap = nullptr;
  ID3D12Resource* readback = nullptr;
  bool unavailable = false;
  bool reported = false;
  uint64_t skipped_in_guest_query = 0;
...
void NbStencilVerifyRelease() {
  ...
  if (v.query_heap) {
    v.query_heap->Release();
    v.query_heap = nullptr;
  }
  v.draws.clear();
}

// Gives the first of the next draw's 24 queries, or false when the check cannot take another draw of this
// shader key.
bool NbStencilVerifyReserve(ID3D12Device* device, uint32_t shader_key, uint32_t* first_query) {
  NbStencilVerifyState& v = nb_stencil_verify;
  if (v.unavailable || v.reported || v.draws.size() >= NbStencilVerifyState::kMaxDraws) {
    return false;
  }
  uint32_t key_draws = 0;
  for (const NbStencilVerifyDraw& draw : v.draws) {
    key_draws += uint32_t(draw.shader_key == shader_key);
  }
  if (key_draws >= NbStencilVerifyState::kMaxDrawsPerKey) {
    return false;
  }
  if (!v.query_heap) {
    D3D12_QUERY_HEAP_DESC heap_desc = {};
    heap_desc.Type = D3D12_QUERY_HEAP_TYPE_OCCLUSION;
    heap_desc.Count = NbStencilVerifyState::kQueriesPerDraw * NbStencilVerifyState::kMaxDraws;
    ...
    if (FAILED(device->CreateQueryHeap(&heap_desc, IID_PPV_ARGS(&v.query_heap))) ||
        FAILED(device->CreateCommittedResource(&heap_properties, D3D12_HEAP_FLAG_NONE, &buffer_desc,
                                               D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                               IID_PPV_ARGS(&v.readback)))) {
      REXGPU_ERROR("nb verify: could not create the stencil check's occlusion queries; nothing was checked");
      NbStencilVerifyRelease();
      v.unavailable = true;
      return false;
    }
  }
  *first_query = uint32_t(v.draws.size()) * NbStencilVerifyState::kQueriesPerDraw;
  return true;
}

// Logs the result once, when the window has ended or the check is full, and every checked draw's submission
// has completed on the GPU.
void NbStencilVerifyReport(bool window_active, uint64_t completed_submission) {
  NbStencilVerifyState& v = nb_stencil_verify;
  if (v.reported || v.draws.empty() || completed_submission < v.last_submission ||
      (window_active && v.draws.size() < NbStencilVerifyState::kMaxDraws)) {
    return;
  }
  v.reported = true;
  const size_t bytes = v.draws.size() * NbStencilVerifyState::kQueriesPerDraw * sizeof(uint64_t);
  D3D12_RANGE read_range = {0, bytes};
  void* mapping = nullptr;
  if (FAILED(v.readback->Map(0, &read_range, &mapping))) {
    REXGPU_ERROR("nb verify: could not map the stencil check's readback; nothing was checked");
    return;
  }
  const uint64_t* counts = static_cast<const uint64_t*>(mapping);
  uint64_t expected = 0, missing = 0, extra = 0, inconsistent = 0;
  uint32_t differing_draws = 0;
  std::vector<std::pair<uint32_t, uint32_t>> keys;  // shader key, checked draws
  for (size_t i = 0; i < v.draws.size(); ++i) {
    const NbStencilVerifyDraw& draw = v.draws[i];
    auto key_it = std::find_if(keys.begin(), keys.end(),
                               [&](const std::pair<uint32_t, uint32_t>& entry) { return entry.first == draw.shader_key; });
    if (key_it == keys.end()) {
      keys.emplace_back(draw.shader_key, 1);
    } else {
      ++key_it->second;
    }
    bool differs = false;
    ...
    for (uint32_t bit = 0; bit < 8; ++bit) {
      const uint64_t* c = counts + draw.first_query + bit * 3;
      const uint64_t sdk_set = c[0], both_set = c[1], dest_set = c[2];
      triples += " " + std::to_string(sdk_set) + "/" + std::to_string(both_set) + "/" + std::to_string(dest_set);
      expected += sdk_set;
      if (both_set > sdk_set || both_set > dest_set) {
        ++inconsistent;
        differs = true;
        continue;
      }
      missing += sdk_set - both_set;
      extra += dest_set - both_set;
      differs |= sdk_set != both_set || dest_set != both_set;
    }
    if (differs && differing_draws++ < 8) {
      REXGPU_WARN("nb verify: draw {} (key {:08X}, {} vertices) differs, A/B/C for bits 0-7:{}", i,
                  draw.shader_key, draw.vertex_count, triples);
    }
  }
  D3D12_RANGE written_range = {0, 0};
  v.readback->Unmap(0, &written_range);
  REXGPU_INFO("nb verify: per-pixel stencil-bit passes against the SDK's in {} draws, {} shader keys, {} more "
              "skipped inside guest occlusion queries: {} set sample-bits expected, {} missing, {} extra, {} "
              "inconsistent counts, {} draws differ",
              v.draws.size(), keys.size(), v.skipped_in_guest_query, expected, missing, extra, inconsistent,
              differing_draws);
  std::string key_list;
  for (const std::pair<uint32_t, uint32_t>& entry : keys) {
    char text[24];
    std::snprintf(text, sizeof(text), " %08X:%u", entry.first, entry.second);
    key_list += text;
  }
...
bool D3D12RenderTargetCache::Initialize() {
  ...
  // nb: the draw extent estimate reads indices and vertices from guest memory. It asks shared memory which
  // ranges hold newer GPU data (util/draw_extent_estimator.cpp, F3). Shutdown withdraws the query.
  SetDrawExtentGpuWrittenQuery(
      [](void* context, uint32_t start, uint32_t length) {
        return static_cast<SharedMemory*>(context)->IsRangeGpuWritten(start, length);
      },
      static_cast<SharedMemory*>(&command_processor_.shared_memory()));
...
bool D3D12RenderTargetCache::Resolve(const memory::Memory& memory, D3D12SharedMemory& shared_memory,
  ...
  if (resolve_info.copy_dest_extent_length) {
    ...
    if (copy_shader != draw_util::ResolveCopyShaderIndex::kUnknown) {
      ...
      D3D12CommandProcessor::NativeGpuPassScope resolve_region(command_processor_,
          "resolve_region_inclusive", uint64_t(copy_shader),
...
// nb: marks the per-pixel stencil-bit variant (nb_transfer_stencil_coverage) in a transfer shader key. The
// SDK's fields use the low 18 bits (render_target_cache.h). Only the copies passed to
// GetOrCreateTransferPipelines carry these bits, so sorting and merging never see them.
constexpr uint32_t kNbTransferStencilCoverageKeyBit = uint32_t(1) << 31;
// The stencil check's pipelines (NbStencilVerifyState), which all write nothing: A, the SDK's stencil-bit
// shader with stencil ALWAYS; B, the same shader with EQUAL on the pass's bit; C, no pixel shader with EQUAL.
constexpr uint32_t kNbTransferStencilVerifyKeyBits[3] = {uint32_t(1) << 30, uint32_t(1) << 29,
                                                         uint32_t(1) << 28};
constexpr uint32_t kNbTransferStencilVerifyKeyMask =
    kNbTransferStencilVerifyKeyBits[0] | kNbTransferStencilVerifyKeyBits[1] | kNbTransferStencilVerifyKeyBits[2];
static_assert(3 * xenos::kMsaaSamplesBits + 2 * xenos::kRenderTargetFormatBits + 1 + 3 <= 28,
              "TransferShaderKey reaches the nb variant bits");
...
ID3D12PipelineState* const* D3D12RenderTargetCache::GetOrCreateTransferPipelines(
  ...
  // nb: the per-pixel variant exists only for stencil bits into a multisampled destination.
  // The stencil check's pipelines use the SDK's shader, at most one check bit per key.
  const bool nb_stencil_coverage = (key.key & kNbTransferStencilCoverageKeyBit) != 0;
  const uint32_t nb_verify_bits = key.key & kNbTransferStencilVerifyKeyMask;
  if ((nb_stencil_coverage || nb_verify_bits) &&
      (!dest_is_stencil_bit || key.dest_msaa_samples == xenos::MsaaSamples::k1X ||
       (nb_stencil_coverage && nb_verify_bits) || (nb_verify_bits & (nb_verify_bits - 1)))) {
    return nullptr;
  }
  // SV_SampleIndex forces sample-rate shading; the per-pixel variant indexes the samples itself.
  const bool nb_sample_index_input =
      key.dest_msaa_samples != xenos::MsaaSamples::k1X && !nb_stencil_coverage;
  ...
  uint32_t isgn_parameter_count = 1 + uint32_t(nb_sample_index_input);
  ...
  uint32_t osgn_parameter_index_sv_coverage =
      nb_stencil_coverage ? osgn_parameter_count++ : UINT32_MAX;
  ...
  uint32_t osgn_sv_coverage_name_ptr = name_ptr;
  if (osgn_parameter_index_sv_coverage != UINT32_MAX) {
    name_ptr += dxbc::AppendAlignedString(built_shader_, "SV_Coverage");
  }
  ...
  // Header and parameters.
  {
    ...
    // SV_Coverage (nb: the per-pixel stencil-bit variant)
    if (osgn_parameter_index_sv_coverage != UINT32_MAX) {
      dxbc::SignatureParameter& osgn_sv_coverage = osgn_parameters[osgn_parameter_index_sv_coverage];
      osgn_sv_coverage.semantic_name_ptr = osgn_sv_coverage_name_ptr;
      osgn_sv_coverage.component_type = dxbc::SignatureRegisterComponentType::kUInt32;
      osgn_sv_coverage.register_index = UINT32_MAX;
      osgn_sv_coverage.mask = 0b0001;
      osgn_sv_coverage.never_writes_mask = 0b1110;
    }
  }
  ...
  // nb: the per-pixel stencil-bit variant gathers its coverage in r3.x.
  a.OpDclTemps(3 + uint32_t(nb_stencil_coverage));
  ...
  // nb: the per-pixel stencil-bit variant emits everything from here to the stencil-bit test once for each
  // sample the pipeline covers, with that sample's index as a literal in place of SV_SampleIndex, and ORs the
  // samples whose bit is set into r3.x. SV_Position has no sample modifier, so it is the pixel centre at
  // either rate and the addressing sees the same pixel. Every other shader runs the body once, as upstream.
  // The body keeps the SDK's indentation so the vendored diff stays reviewable.
  uint32_t nb_coverage_samples[4] = {0, 1, 2, 3};
  uint32_t nb_coverage_sample_count = 1;
  if (nb_stencil_coverage) {
    if (key.dest_msaa_samples == xenos::MsaaSamples::k2X) {
      // Native 2x is samples 0 and 1; 2x emulated as 4x is samples 0 and 3 (SampleMask 0b1001 below).
      nb_coverage_samples[1] = msaa_2x_supported_ ? 1 : 3;
      nb_coverage_sample_count = 2;
    } else {
      nb_coverage_sample_count = 4;
    }
    a.OpMov(dxbc::Dest::R(3, 0b0001), dxbc::Src::LU(0));
  }
  for (uint32_t nb_coverage_index = 0; nb_coverage_index < nb_coverage_sample_count;
       ++nb_coverage_index) {
  const uint32_t nb_coverage_sample = nb_coverage_samples[nb_coverage_index];
  ...
  dxbc::Src dest_sample(nb_stencil_coverage
                            ? dxbc::Src::LU(nb_coverage_sample)
  ...
  if (!source_is_64bpp && dest_is_64bpp) {
    ...
    if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
      ...
      if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        a.OpBFI(dxbc::Dest::R(1, 0b0001), dxbc::Src::LU(1), dxbc::Src::LU(1), dest_sample,
      ...
      case TransferOutput::kStencilBit:
        ...
        a.OpAnd(dxbc::Dest::R(0, 0b0001), dxbc::Src::R(1, dxbc::Src::kXXXX),
                dxbc::Src::CB(cbuffer_index_stencil_mask, kTransferCBVRegisterStencilMask, 0,
                              dxbc::Src::kXXXX));
        if (nb_stencil_coverage) {
          // nb: instead, add this sample to the coverage if its bit is set.
          a.OpMovC(dxbc::Dest::R(0, 0b0001), dxbc::Src::R(0, dxbc::Src::kXXXX),
                   dxbc::Src::LU(uint32_t(1) << nb_coverage_sample), dxbc::Src::LU(0));
          a.OpOr(dxbc::Dest::R(3, 0b0001), dxbc::Src::R(3, dxbc::Src::kXXXX),
                 dxbc::Src::R(0, dxbc::Src::kXXXX));
        } else {
  ...
  }  // nb: end of the per-sample body.
  if (nb_stencil_coverage) {
    // Write the bit to the samples that have it; a pixel where none has it writes nothing.
    a.OpDiscard(false, dxbc::Src::R(3, dxbc::Src::kXXXX));
    a.OpMov(dxbc::Dest::OMask(), dxbc::Src::R(3, dxbc::Src::kXXXX));
  }
  ...
  if (dest_is_stencil_bit) {
    ...
    // nb: the stencil check's pipelines write nothing; B and C pass where the destination has the bit.
    if (nb_verify_bits) {
      D3D12_DEPTH_STENCILOP_DESC& face = pipeline_desc.DepthStencilState.FrontFace;
      face.StencilPassOp = D3D12_STENCIL_OP_KEEP;
      face.StencilFunc = nb_verify_bits == kNbTransferStencilVerifyKeyBits[0] ? D3D12_COMPARISON_FUNC_ALWAYS
                                                                             : D3D12_COMPARISON_FUNC_EQUAL;
      pipeline_desc.DepthStencilState.BackFace = face;
      if (nb_verify_bits == kNbTransferStencilVerifyKeyBits[2]) {
        pipeline_desc.PS = {};
      }
    }
    ...
    for (uint32_t i = 0; i < 8; ++i) {
      if (nb_verify_bits) {
        pipeline_desc.DepthStencilState.StencilReadMask = UINT8(1) << i;
    ...
    if ((nb_stencil_coverage || nb_verify_bits) && pipelines) {
      REXGPU_INFO("nb: created {} transfer pipelines, key {:08X}: {}-sample source, "
                  "{}-sample depth format {}, mode {}",
                  nb_stencil_coverage ? "per-pixel stencil-bit" : "stencil check", key.key,
                  uint32_t(1) << uint32_t(key.source_msaa_samples),
                  uint32_t(1) << uint32_t(key.dest_msaa_samples), uint32_t(key.dest_resource_format),
                  uint32_t(key.mode));
    }
  } else {
    if (dest_is_color) {
...
void D3D12RenderTargetCache::PerformTransfersAndResolveClears(
  ...
  // inside one, which biased the first UW15 timing screens (docs/ultrawide-uw15-stencil-coverage.md).
  static uint64_t nb_windows_frame = UINT64_MAX;
  static bool nb_skip_all_transfers = false;
  static bool nb_skip_transfer_stencil = false;
  static bool nb_verify_stencil_coverage = false;
  static bool nb_stencil_coverage_window = false;
  const uint64_t nb_current_frame = NbGetCompletedSwapCount() + 1;
  if (nb_current_frame != nb_windows_frame) {
    nb_windows_frame = nb_current_frame;
    nb_skip_all_transfers = NbFrameInWindows(REXCVAR_GET(nb_diag_skip_transfers_frames));
    nb_skip_transfer_stencil = NbFrameInWindows(REXCVAR_GET(nb_diag_skip_transfer_stencil_frames));
    nb_verify_stencil_coverage = NbFrameInWindows(REXCVAR_GET(nb_transfer_stencil_coverage_verify_frames));
    nb_stencil_coverage_window = NbFrameInWindows(REXCVAR_GET(nb_transfer_stencil_coverage_frames));
  }
  static bool nb_logged_skip_all = false;
  static bool nb_logged_skip_stencil = false;
  if (nb_skip_all_transfers && !nb_logged_skip_all) {
    nb_logged_skip_all = true;
    REXGPU_WARN("nb diag: every EDRAM transfer draw skipped from frame {} (nb_diag_skip_transfers_frames {}); "
                "the output is wrong in that window",
                NbGetCompletedSwapCount() + 1, REXCVAR_GET(nb_diag_skip_transfers_frames));
  }
  if (nb_skip_transfer_stencil && !nb_logged_skip_stencil) {
    nb_logged_skip_stencil = true;
    REXGPU_WARN("nb diag: depth-transfer stencil passes skipped from frame {} "
                "(nb_diag_skip_transfer_stencil_frames {}); the output is wrong in that window",
                NbGetCompletedSwapCount() + 1, REXCVAR_GET(nb_diag_skip_transfer_stencil_frames));
  }
  // nb: per-pixel stencil-bit passes for multisampled destinations (nb_transfer_stencil_coverage). Each
  // switch is logged with the count of transfers that used them so far, so a same-run window shows it ran.
  const bool nb_stencil_coverage =
      REXCVAR_GET(nb_transfer_stencil_coverage) || nb_verify_stencil_coverage || nb_stencil_coverage_window;
  NbStencilVerifyReport(nb_verify_stencil_coverage, command_processor_.GetCompletedSubmission());
  static bool nb_stencil_coverage_logged_state = false;
  static uint64_t nb_stencil_coverage_transfers = 0;
  if (nb_stencil_coverage != nb_stencil_coverage_logged_state) {
    nb_stencil_coverage_logged_state = nb_stencil_coverage;
    REXGPU_INFO("nb: per-pixel stencil-bit transfers {} from frame {} ({} multisampled stencil-bit "
  ...
  for (uint32_t i = 0; i < render_target_count; ++i) {
    ...
    if (!current_transfers.empty() && !nb_skip_all_transfers) {
      ...
      bool need_stencil_bit_draws =
          dest_rt_key.is_depth && !use_stencil_reference_output_ && !nb_skip_transfer_stencil;
      ...
      for (auto it = current_transfer_invocations_.cbegin();
        ...
        // nb: the per-pixel stencil-bit variant, falling back to the SDK's passes if it fails to build.
        TransferShaderKey transfer_pipelines_key = transfer_shader_key;
        if (is_stencil_bit && nb_stencil_coverage &&
            transfer_shader_key.dest_msaa_samples != xenos::MsaaSamples::k1X) {
          transfer_pipelines_key.key |= kNbTransferStencilCoverageKeyBit;
        }
        ID3D12PipelineState* const* transfer_pipelines =
            GetOrCreateTransferPipelines(transfer_pipelines_key);
        if (!transfer_pipelines && transfer_pipelines_key != transfer_shader_key) {
          transfer_pipelines_key = transfer_shader_key;
          transfer_pipelines = GetOrCreateTransferPipelines(transfer_pipelines_key);
        }
        if (!transfer_pipelines) {
          continue;
        }
        const bool nb_transfer_uses_stencil_coverage = transfer_pipelines_key != transfer_shader_key;
        nb_stencil_coverage_transfers += uint64_t(nb_transfer_uses_stencil_coverage);
        ...
        command_processor_.SubmitBarriers();
        D3D12CommandProcessor::NativeGpuPassScope transfer_scope(command_processor_,
        ...
        if (nb_transfer_uses_stencil_coverage && nb_verify_stencil_coverage) {
          if (command_processor_.IsGuestOcclusionQueryActive()) {
            ++nb_stencil_verify.skipped_in_guest_query;
          } else {
            std::array<ID3D12PipelineState*, 8> verify_pipelines[3];
            bool verify_pipelines_ready = true;
            for (uint32_t k = 0; k < 3 && verify_pipelines_ready; ++k) {
              TransferShaderKey verify_key = transfer_shader_key;
              verify_key.key |= kNbTransferStencilVerifyKeyBits[k];
              ID3D12PipelineState* const* pipelines = GetOrCreateTransferPipelines(verify_key);
              if (pipelines) {
                std::copy(pipelines, pipelines + 8, verify_pipelines[k].begin());
              } else {
                verify_pipelines_ready = false;
              }
            }
            uint32_t first_query;
            if (verify_pipelines_ready &&
                NbStencilVerifyReserve(command_processor_.GetD3D12Provider().GetDevice(),
                                       transfer_shader_key.key, &first_query)) {
              for (uint32_t bit = 0; bit < 8; ++bit) {
                uint32_t transfer_stencil_bit = uint32_t(1) << bit;
                command_list.D3DSetGraphicsRoot32BitConstants(
                    rex::bit_count(transfer_root_parameters_used &
                                   (kTransferUsedRootParameterStencilMaskConstantBit - 1)),
                    sizeof(transfer_stencil_bit) / sizeof(uint32_t), &transfer_stencil_bit, 0);
                for (uint32_t k = 0; k < 3; ++k) {
                  const uint32_t query = first_query + bit * 3 + k;
                  command_list.D3DBeginQuery(nb_stencil_verify.query_heap, D3D12_QUERY_TYPE_OCCLUSION, query);
                  command_processor_.SetExternalPipeline(verify_pipelines[k][bit]);
                  command_list.D3DDrawInstanced(transfer_vertex_count, 1, 0, 0);
                  command_list.D3DEndQuery(nb_stencil_verify.query_heap, D3D12_QUERY_TYPE_OCCLUSION, query);
                }
              }
              command_list.D3DResolveQueryData(nb_stencil_verify.query_heap, D3D12_QUERY_TYPE_OCCLUSION,
                                               first_query, NbStencilVerifyState::kQueriesPerDraw,
                                               nb_stencil_verify.readback,
                                               uint64_t(first_query) * sizeof(uint64_t));
              nb_stencil_verify.draws.push_back({first_query, transfer_shader_key.key, transfer_vertex_count});
              nb_stencil_verify.last_submission = current_submission;
            }
          }
        }
      }
    ...
    if (resolve_clear_needed) {
      ...
      if (dest_rt_key.is_depth) {
        ...
        command_processor_.SubmitBarriers();
        {
          D3D12CommandProcessor::NativeGpuPassScope stencil_clear_scope(command_processor_, "transfer_stencil_clear");
...
bool D3D12RenderTargetCache::DumpRenderTargets(uint32_t dump_base, uint32_t dump_row_length_used,
  ...
  // Rectangle count conservatively screens ownership splits, not unique owners.
  const auto dump_key = dump_rectangles_.front().render_target->key();
  D3D12CommandProcessor::NativeGpuPassScope dump_scope(command_processor_, "resolve_dump",
      uint64_t(dump_key.resource_format) | (uint64_t(dump_key.msaa_samples) << 32) |
          (uint64_t(dump_key.is_depth) << 40), dump_rectangles_.size(), dump_rows);
  // nb: Phase 3 item 6 named counter. One dump = the EDRAM-dump half of one
  // host-RT resolve (the round trip step 3 wants to eliminate).
  ++GetPhase3Counters().resolve_dump_count;
