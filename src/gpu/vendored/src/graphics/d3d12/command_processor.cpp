...
#include "native/native_gpu_submission_timing.h"
...
REXCVAR_DEFINE_BOOL(nb_native_async_replay, false, "nb",
                    "Overlap non-closing deferred replay with later CPU preparation, preserving queue and fence order")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
REXCVAR_DEFINE_INT32(nb_native_replay_chunk_draws, 0, "nb",
                    "Experimental CPU replay chunks within one unchanged GPU submission; zero disables")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
// Default-off production-replay budget probe. Time is relative to the first
// recorded swap frame; FPS/control windows must end before this diagnostic phase.
REXCVAR_DEFINE_BOOL(nb_gpu_budget_probe, false, "nb", "Enable the bounded async-preserving GPU budget probe");
REXCVAR_DEFINE_STRING(nb_gpu_budget_file, "", "nb", "Output JSON for the GPU budget probe");
REXCVAR_DEFINE_UINT32(nb_gpu_budget_start_seconds, 170, "nb", "Seconds after the first frame before GPU diagnostics");
REXCVAR_DEFINE_UINT32(nb_gpu_budget_frames, 1200, "nb", "Finite GPU budget capture length (1 to 2400 frames)");
REXCVAR_DEFINE_UINT32(nb_gpu_budget_group_period, 200, "nb", "Group attribution period; zero records submission brackets only");

REXCVAR_DEFINE_STRING(nb_native_gpu_timing_file, "", "nb",
                      "Opt-in bounded backend GPU timestamp capture, written after shutdown drain")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);
REXCVAR_DEFINE_INT32(nb_native_gpu_timing_start, 12000, "nb",
                     "First one-based native frame number in GPU timestamp capture")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);
REXCVAR_DEFINE_INT32(nb_native_gpu_timing_frames, 1200, "nb",
                     "Number of frame counters in bounded GPU timestamp capture")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);

REXCVAR_DEFINE_UINT32(nb_native_gpu_pass_timing_period, 0, "nb",
                      "Sample one GPU pass-timing frame per N capture frames; zero disables; requires GPU timing file")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);
REXCVAR_DEFINE_BOOL(nb_native_gpu_pass_timing_geometry, false, "nb",
                    "Highly perturbing per-draw GPU timestamps; normally leave off and attribute copies only")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);

REXCVAR_DEFINE_BOOL(nb_native_defer_guest_pipeline, true, "nb",
                    "Defer the emulated pipeline bind until native refusal and reuse the texture request at the native seam")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_static_bindings, true, "nb",
                    "Reuse unchanged native geometry texture, sampler and buffer root bindings")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_pixel_binding_cache, false, "nb",
                    "Reuse the exact native pixel constant-buffer GPU address at root 4")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_skip_guest_pipeline, true, "nb",
                    "Prepare shader bindings before native draws and select an emulated pipeline only on native refusal")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_BOOL(nb_native_hardware_indices, false, "nb",
                    "Use the hardware index buffer for unchanged native DMA triangle lists; false keeps shader index gathering")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);
...
REXCVAR_DEFINE_STRING(nb_phase3_counters_file, "", "nb",
                      "JSON-lines output path for the Phase 3 per-frame counter set");

namespace rex::graphics::d3d12 {

namespace {
// Original count-only attribution for one IssueDraw. No resource state, barrier
// order or memory authority is changed; all observations are queued transitions.
enum class NativeUploadPhase { kOther, kTextures, kNative };
struct NativeUploadObservation {
  const D3D12CommandProcessor* owner = nullptr;
  NativeSubmissionStats* stats = nullptr;
  NativeUploadPhase phase = NativeUploadPhase::kOther;
  bool interrupted = false;
  bool texture_read_seen = false;
  bool texture_read_pending = false;
  bool native_read_pending = false;
  uint64_t native_copy_dest = 0;
  uint64_t late_copy_dest = 0;
  uint64_t completed_cycles = 0;

  void Transition(D3D12_RESOURCE_STATES before, D3D12_RESOURCE_STATES after) noexcept {
    const bool was_shader_read =
        (before & (D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE |
                   D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)) != 0;
    const bool is_shader_read =
        (after & D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE) != 0;
    const bool was_copy_dest = (before & D3D12_RESOURCE_STATE_COPY_DEST) != 0;
    const bool is_copy_dest = (after & D3D12_RESOURCE_STATE_COPY_DEST) != 0;
    if (phase == NativeUploadPhase::kTextures) {
      // A conversion may have several uploads/reads. The last actual state
      // must still be a shader read before the later native upload qualifies.
      if (is_shader_read && !(before & D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)) {
        texture_read_seen = true;
        texture_read_pending = true;
      } else if (!is_shader_read) {
        texture_read_pending = false;
      }
      return;
    }
    if (phase == NativeUploadPhase::kNative) {
      if (native_read_pending) {
        if (was_copy_dest && is_shader_read) ++completed_cycles;
        native_read_pending = false;
      }
      if (!was_copy_dest && is_copy_dest) {
        ++native_copy_dest;
        if (texture_read_pending && was_shader_read) {
          ++late_copy_dest;
          native_read_pending = true;
        }
      }
      // Only the texture phase may establish this candidate's first read.
      texture_read_pending = false;
      return;
    }
    // An intervening shared-buffer operation outside both phases makes the
    // proposed adjacency uncertain, so retain no cross-phase candidate.
    texture_read_pending = false;
    native_read_pending = false;
  }
};
thread_local NativeUploadObservation* active_native_upload_observation = nullptr;

class NativeUploadDrawScope {
 public:
  NativeUploadDrawScope(const D3D12CommandProcessor* owner, bool enabled) noexcept
      : previous_(active_native_upload_observation) {
    // A nested packet may use the same resource between the parent's phases.
    // Attribute it independently and conservatively omit the parent's pattern.
    if (previous_) previous_->interrupted = true;
    if (enabled) {
      observation_.emplace();
      observation_->owner = owner;
      observation_->stats = &GetNativeSubmissionStats();
      active_native_upload_observation = &*observation_;
    } else if (previous_) {
      active_native_upload_observation = nullptr;
    }
  }
  ~NativeUploadDrawScope() {
    if (observation_ || previous_) active_native_upload_observation = previous_;
  }
  NativeUploadDrawScope(const NativeUploadDrawScope&) = delete;
  NativeUploadDrawScope& operator=(const NativeUploadDrawScope&) = delete;
  NativeUploadObservation* observation() noexcept {
    return observation_ ? &*observation_ : nullptr;
  }
  void CompleteNative(bool recorded, bool texture_request_deduplicated) noexcept {
    if (!observation_) return;
    const auto& observation = *observation_;
    auto& stats = *observation.stats;
    ++stats.upload_order_native_attempts;
    stats.upload_order_native_draws += recorded;
    if (observation.interrupted || !texture_request_deduplicated) {
      ++stats.upload_order_attribution_bypasses;
      return;
    }
    if (!recorded) {
      stats.upload_order_refused_candidate_draws += observation.late_copy_dest != 0;
      return;
    }
    stats.upload_order_texture_read_draws += observation.texture_read_seen;
    stats.upload_order_copy_dest_draws += observation.native_copy_dest != 0;
    stats.upload_order_candidate_draws += observation.late_copy_dest != 0;
    stats.upload_order_late_copy_dest_transitions += observation.late_copy_dest;
    stats.upload_order_candidate_cycles += observation.completed_cycles;
  }

 private:
  std::optional<NativeUploadObservation> observation_;  // No packet initialization when diagnostics are off.
  NativeUploadObservation* previous_;
};

class NativeUploadPhaseScope {
 public:
  NativeUploadPhaseScope(NativeUploadObservation* observation, NativeUploadPhase phase) noexcept
      : observation_(observation), previous_(observation ? observation->phase : NativeUploadPhase::kOther) {
    if (observation_) observation_->phase = phase;
  }
  ~NativeUploadPhaseScope() { if (observation_) observation_->phase = previous_; }
  NativeUploadPhaseScope(const NativeUploadPhaseScope&) = delete;
  NativeUploadPhaseScope& operator=(const NativeUploadPhaseScope&) = delete;

 private:
  NativeUploadObservation* observation_;
  NativeUploadPhase previous_;
};
}  // namespace
...
bool D3D12CommandProcessor::PushTransitionBarrier(ID3D12Resource* resource,
  ...
  if (auto* observation = active_native_upload_observation;
      observation && observation->owner == this && shared_memory_ &&
      resource == shared_memory_->GetBuffer()) {
    observation->Transition(old_state, new_state);
  }
  return true;
}
...
void D3D12CommandProcessor::SubmitBarriers() {
  ...
  if (barrier_count != 0) {
    ...
    if (REXCVAR_GET(nb_native_submission_diagnostics)) {
      auto& stats = GetNativeSubmissionStats();
      ++stats.barrier_batches;
      stats.barriers_total += barrier_count;
      stats.max_barriers_per_batch = std::max(stats.max_barriers_per_batch, uint64_t(barrier_count));
      const ID3D12Resource* shared_resource = shared_memory_ ? shared_memory_->GetBuffer() : nullptr;
      for (const D3D12_RESOURCE_BARRIER& barrier : barriers_) {
        switch (barrier.Type) {
          case D3D12_RESOURCE_BARRIER_TYPE_TRANSITION:
            ++stats.transition_barriers;
            if (shared_resource && barrier.Transition.pResource == shared_resource) {
              ++stats.shared_memory_transitions;
              const bool was_copy_dest =
                  (barrier.Transition.StateBefore & D3D12_RESOURCE_STATE_COPY_DEST) != 0;
              const bool is_copy_dest =
                  (barrier.Transition.StateAfter & D3D12_RESOURCE_STATE_COPY_DEST) != 0;
              if (!was_copy_dest && is_copy_dest) ++stats.shared_memory_to_copy_dest;
              if (was_copy_dest && !is_copy_dest) ++stats.shared_memory_from_copy_dest;
            }
            break;
...
uint64_t D3D12CommandProcessor::RequestViewBindfulDescriptors(
  ...
  if (view_bindful_heap_current_ != heap) {
    ...
    native_static_bindings_.InvalidateDescriptorTables();
...
bool D3D12CommandProcessor::RotateSamplerBindlessHeap() {
  assert_true(bindless_resources_used_);
  assert_true(submission_open_);
  // An allocation attempt may already have overwritten part of the emulated index arrays. Invalidate
  // both stages even if creating the replacement fails, matching UpdateBindings' overflow contract.
  ...
  if (!sampler_bindless_heaps_overflowed_.empty() &&
    ...
    desc.NumDescriptors = kSamplerHeapSize;
    desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    if (FAILED(GetD3D12Provider().GetDevice()->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&sampler_heap_new)))) {
      REXGPU_ERROR("Failed to create a new bindless sampler descriptor heap after an overflow");
      return false;
    }
  }
  // Retain the old heap until every submitted or deferred draw referring to it has completed.
  ...
  sampler_bindless_heap_cpu_start_ = sampler_heap_new->GetCPUDescriptorHandleForHeapStart();
  sampler_bindless_heap_gpu_start_ = sampler_heap_new->GetGPUDescriptorHandleForHeapStart();
  sampler_bindless_heap_allocated_ = 0;
  texture_cache_bindless_sampler_map_.clear();
  // Invalidate even when a recycled heap has the same GPU handle as an older
  // native draw. Pointer/handle equality does not describe heap generations.
  native_static_bindings_.InvalidateDescriptorTables();
...
bool D3D12CommandProcessor::RequestSamplerBindlessIndices(
    const D3D12TextureCache::SamplerParameters* parameters, uint32_t count, uint32_t* indices_out) {
  assert_true(submission_open_);
  if (!bindless_resources_used_ || count > kSamplerHeapSize) {
    return false;
  }
  const auto& provider = GetD3D12Provider();
  for (uint32_t attempt = 0; attempt < 2; ++attempt) {
    if (attempt && !RotateSamplerBindlessHeap()) {
      return false;
    }
    uint32_t i = 0;
    for (; i < count; ++i) {
      auto found = texture_cache_bindless_sampler_map_.find(parameters[i].value);
      if (found != texture_cache_bindless_sampler_map_.end()) {
        indices_out[i] = found->second;
        continue;
      }
      if (sampler_bindless_heap_allocated_ >= kSamplerHeapSize) {
        break;
      }
      const uint32_t index = sampler_bindless_heap_allocated_++;
      texture_cache_->WriteSampler(parameters[i],
                                  provider.OffsetSamplerDescriptor(sampler_bindless_heap_cpu_start_, index));
      texture_cache_bindless_sampler_map_.emplace(parameters[i].value, index);
      indices_out[i] = index;
    }
    if (i == count) {
      return true;
    }
    // Retry ALL indices, including cache hits from the first attempt, against the replacement heap.
  }
  return false;
}
...
void D3D12CommandProcessor::SetExternalPipeline(ID3D12PipelineState* pipeline) {
  if (REXCVAR_GET(nb_native_pipeline_binding_diagnostics)) {
    auto& stats = GetNativeSubmissionStats();
    if (current_external_pipeline_ != pipeline) ++stats.external_pipeline_binds;
    else ++stats.external_pipeline_reuses;
  }
...
void D3D12CommandProcessor::SetExternalGraphicsRootSignature(ID3D12RootSignature* root_signature) {
  // An external pass may overwrite roots even when it uses the same signature.
  native_static_bindings_.InvalidateAll();
...
DeferredCommandList& D3D12CommandProcessor::SetNativeGeometryStaticBindings(
    ID3D12RootSignature* root_signature, D3D12_GPU_DESCRIPTOR_HANDLE textures,
    D3D12_GPU_DESCRIPTOR_HANDLE samplers, bool sampler_used,
    D3D12_GPU_VIRTUAL_ADDRESS guest_memory, D3D12_GPU_VIRTUAL_ADDRESS asset_memory) {
  assert_true(submission_open_);
  assert_not_null(root_signature);
  if (current_graphics_root_signature_ != root_signature) {
    native_static_bindings_.InvalidateAll();
    current_graphics_root_signature_ = root_signature;
    deferred_command_list_.D3DSetGraphicsRootSignature(root_signature);
  }
  // Native root writes invalidate the emulated root cache, just as the ordinary
  // external-signature helper does, including when the signature is unchanged.
  current_graphics_root_up_to_date_ = 0;
  const uint32_t changed = native_static_bindings_.Update(
      reinterpret_cast<uintptr_t>(root_signature),
      {textures.ptr, samplers.ptr, guest_memory, asset_memory}, sampler_used,
      REXCVAR_GET(nb_native_static_bindings));
  if (changed & NativeStaticBindings::kTextureTable) {
    deferred_command_list_.D3DSetGraphicsRootDescriptorTable(2, textures);
  }
  if (changed & NativeStaticBindings::kSamplerTable) {
    deferred_command_list_.D3DSetGraphicsRootDescriptorTable(5, samplers);
  }
  if (changed & NativeStaticBindings::kGuestMemory) {
    deferred_command_list_.D3DSetGraphicsRootShaderResourceView(3, guest_memory);
  }
  if (changed & NativeStaticBindings::kAssetMemory) {
    deferred_command_list_.D3DSetGraphicsRootShaderResourceView(6, asset_memory);
  }
  return deferred_command_list_;
}

void D3D12CommandProcessor::SetNativePixelConstantBuffer(D3D12_GPU_VIRTUAL_ADDRESS address) {
  assert_true(submission_open_);
  assert_not_null(current_graphics_root_signature_);
  current_graphics_root_up_to_date_ = 0;
  const bool enabled = REXCVAR_GET(nb_native_pixel_binding_cache);
  if (!native_static_bindings_.NeedsPixelConstantBufferWrite(
          reinterpret_cast<uintptr_t>(current_graphics_root_signature_), address, enabled)) {
    return;
  }
  deferred_command_list_.D3DSetGraphicsRootConstantBufferView(4, address);
  native_static_bindings_.DidWritePixelConstantBuffer(address, enabled);
}

void D3D12CommandProcessor::SetNativeGeometryConstantBuffers(
    D3D12_GPU_VIRTUAL_ADDRESS root0, D3D12_GPU_VIRTUAL_ADDRESS vertex,
    D3D12_GPU_VIRTUAL_ADDRESS pixel) {
  assert_true(submission_open_);
  assert_not_null(current_graphics_root_signature_);
  // Native writes invalidate the emulated roots even when the pixel CBV is
  // already current. Roots0/1 are always written at this exact draw position.
  current_graphics_root_up_to_date_ = 0;
  const bool enabled = REXCVAR_GET(nb_native_pixel_binding_cache);
  const bool write_pixel = native_static_bindings_.NeedsPixelConstantBufferWrite(
      reinterpret_cast<uintptr_t>(current_graphics_root_signature_), pixel, enabled);
  deferred_command_list_.D3DSetNativeGraphicsConstantBufferViews(root0, vertex, pixel, write_pixel);
  // Appending may allocate. Never publish a pixel binding if it did not finish.
  if (write_pixel) native_static_bindings_.DidWritePixelConstantBuffer(pixel, enabled);
  ++native_constant_binding_packet_stats_.packets;
  native_constant_binding_packet_stats_.pixel_writes += write_pixel;
}
...
uint32_t D3D12CommandProcessor::BeginNativeGpuPass(const char* name,
    uint64_t detail0, uint64_t detail1, uint64_t detail2) {
  return native_gpu_submission_timing_ && submission_open_
      ? native_gpu_submission_timing_->BeginPass(deferred_command_list_, NativeGpuTimingFrame(),
          submission_current_, name, detail0, detail1, detail2)
      : nb::gpu::NativeGpuSubmissionTiming::kNoSlot;
}
void D3D12CommandProcessor::EndNativeGpuPass(uint32_t slot) {
  if (native_gpu_submission_timing_) native_gpu_submission_timing_->EndPass(deferred_command_list_, slot);
}

uint32_t D3D12CommandProcessor::BeginNativeBudgetScope(const char* name, uint64_t detail0) {
  return native_gpu_budget_probe_ && native_gpu_budget_probe_->scopes_active()
      ? native_gpu_budget_probe_->EnterScope(deferred_command_list_, name, detail0) : UINT32_MAX;
}
void D3D12CommandProcessor::EndNativeBudgetScope(uint32_t previous) {
  if (native_gpu_budget_probe_) native_gpu_budget_probe_->LeaveScope(deferred_command_list_, previous);
}
...
bool D3D12CommandProcessor::SetupContext() {
  ...
  const std::string timing_path = REXCVAR_GET(nb_native_gpu_timing_file);
  if (!timing_path.empty()) {
    const int32_t first = REXCVAR_GET(nb_native_gpu_timing_start);
    const int32_t count = REXCVAR_GET(nb_native_gpu_timing_frames);
    if (first >= 0 && count > 0) {
      try {
        native_gpu_submission_timing_ = nb::gpu::NativeGpuSubmissionTiming::Create(
            device, provider.GetDirectQueue(), timing_path, uint64_t(first), uint64_t(count));
        if (native_gpu_submission_timing_ && !native_gpu_submission_timing_->EnablePassTiming(
                REXCVAR_GET(nb_native_gpu_pass_timing_period), REXCVAR_GET(nb_native_gpu_pass_timing_geometry))) {
          REXGPU_WARN("Native GPU pass timing could not initialize; submission capture only");
        }
      } catch (const std::exception&) {
        // Optional diagnostics must not prevent rendering on allocation failure.
      }
    }
    if (!native_gpu_submission_timing_) REXGPU_WARN("Native GPU timestamp capture could not initialize");
  }
  if (REXCVAR_GET(nb_gpu_budget_probe)) {
    // Mutually exclusive with the intrusive legacy timing/submission diagnostics.
    // A refused probe leaves rendering on its original path and logs the refusal.
    if (!timing_path.empty() || REXCVAR_GET(nb_native_submission_diagnostics) ||
        REXCVAR_GET(nb_native_replay_chunk_draws) ||
        !REXCVAR_GET(nb_phase3_counters_file).empty()) {
      REXGPU_WARN("NB_GPU budget probe refused: legacy timing, submission diagnostics, chunks or Phase 3 file logging are enabled");
    } else {
      try {
        native_gpu_budget_probe_ = nb::gpu::NativeGpuBudgetProbe::Create(
            device, direct_queue, REXCVAR_GET(nb_gpu_budget_file),
            REXCVAR_GET(nb_gpu_budget_start_seconds), REXCVAR_GET(nb_gpu_budget_frames),
            REXCVAR_GET(nb_gpu_budget_group_period));
      } catch (const std::exception&) {
        // Optional diagnostics never turn allocation failure into a rendering failure.
      }
      REXGPU_INFO("NB_GPU budget probe {}; start {} seconds, {} frames, group period {}; async replay gate unchanged",
          native_gpu_budget_probe_ ? "ready" : "unavailable",
          REXCVAR_GET(nb_gpu_budget_start_seconds), REXCVAR_GET(nb_gpu_budget_frames),
          REXCVAR_GET(nb_gpu_budget_group_period));
    }
  }
...
void D3D12CommandProcessor::ShutdownContext() {
  ...
  native_replay_worker_.reset();
  if (native_gpu_submission_timing_) {
    const uint64_t completed = submission_fence_ ? submission_fence_->GetCompletedValue() : UINT64_MAX;
    if (!native_gpu_submission_timing_->finished() &&
        !native_gpu_submission_timing_->Finish(native_timing_idle, completed)) {
      REXGPU_WARN("Native GPU timestamp capture incomplete or could not be written");
    }
    native_gpu_submission_timing_.reset();
  }
  deferred_command_list_.SetNativeBudgetProbe(nullptr);
  if (native_gpu_budget_probe_) {
    const uint64_t completed = submission_fence_ ? submission_fence_->GetCompletedValue() : UINT64_MAX;
    if (!native_gpu_budget_probe_->finished() &&
        !native_gpu_budget_probe_->Finish(native_timing_idle, completed)) {
      REXGPU_WARN("NB_GPU budget probe incomplete or could not be written");
    }
    native_gpu_budget_probe_.reset();
  }
  InvalidateAllVertexBufferResidency();
...
void D3D12CommandProcessor::IssueSwap(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
  ...
  // nb: display-aspect seam (see the header and src/gpu/vendored/UPSTREAM.md).
  OverrideDisplayAspect(display_width, display_height);
  ...
  presenter->RefreshGuestOutput(
      ...
      [this, &swap_texture_srv_desc, frontbuffer_format, swap_texture_resource, guest_output_width,
       guest_output_height](ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        ...
        // queue.
        SubmitBarriers();
        if (!EndSubmission(true)) return false;
        return true;
      });

  // End the frame even if did not present for any reason (the image refresher
  // was not called), to prevent leaking per-frame resources.
  if (!EndSubmission(true)) return;
}
...
bool D3D12CommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type, uint32_t index_count,
  ...
  NativeUploadDrawScope upload_order_scope(this, REXCVAR_GET(nb_native_submission_diagnostics));
  ...
  {
    nb::gpu::NativeRangeCallerScope origin(nb::gpu::NativeRangeCaller::kIndexProcessing,
                                           nb::gpu::NativeRangeDiagnosticsEnabled());
  ...
  // Prepare shader translations now to obtain the used textures. The emulated
  // pipeline needs the bound target formats, but an eligible native draw can
  // defer its selection until the native pass refuses the draw.
  ...
  const bool skip_guest_pipeline =
      REXCVAR_GET(nb_native_skip_guest_pipeline) && bindless_resources_used_ &&
      host_render_targets_used && !memexport_used && is_rasterization_done &&
      !primitive_processing_result.IsTessellated();
  auto configure_guest_pipeline = [&]() {
  ...
  if (skip_guest_pipeline) {
    if (!pipeline_cache_->PrepareShaderTranslations(vertex_shader_translation,
                                                    pixel_shader_translation)) {
      return false;
    }
    // GetCurrentStateDescription rejects only a pixel shader with rasterization
    // disabled, or a null root signature. Eligibility above excludes the former;
    // preserve the exact latter check without constructing the emulated PSO key.
    // Bindless signatures were created during SetupContext, so this is a lookup.
    root_signature = GetRootSignature(vertex_shader, pixel_shader,
                                      primitive_processing_result.IsTessellated());
    if (!root_signature) return false;
  } else if (!configure_guest_pipeline()) {
    return false;
  }
  ...
  {
    NativeUploadPhaseScope phase(upload_order_scope.observation(), NativeUploadPhase::kTextures);
    nb::gpu::NativeRangeCallerScope origin(nb::gpu::NativeRangeCaller::kTextureRequest,
                                           nb::gpu::NativeRangeDiagnosticsEnabled());
    texture_cache_->RequestTextures(used_texture_mask);
  }

  // The native seam below uses its own pipeline. Retain the old bind order for
  // the diagnostic comparison; otherwise bind only when the emulated draw is
  // actually needed. Skipping emulated pipeline selection also requires deferring
  // its bind, independently of the older bind-only diagnostic option.
  const bool defer_guest_pipeline =
      skip_guest_pipeline || REXCVAR_GET(nb_native_defer_guest_pipeline);
  if (!defer_guest_pipeline && current_guest_pipeline_ != pipeline_handle) {
  ...
  // nb: native-draw seam (NativeDrawContext in the header). Only on the host render target path,
  // where the bound targets are real D3D12 render targets a native pipeline can draw into.
  if (host_render_targets_used && !memexport_used) {
    NativeDrawContext native_context;
    native_context.guest_primitive_type = primitive_type;
    native_context.index_count = index_count;
    native_context.indexed = primitive_processing_result.index_buffer_type !=
                             PrimitiveProcessor::ProcessedIndexBufferType::kNone;
    native_context.index_guest_base = index_buffer_info ? index_buffer_info->guest_base : 0;
    native_context.index_format = index_buffer_info ? index_buffer_info->format : xenos::IndexFormat::kInt16;
    native_context.index_endian = index_buffer_info ? index_buffer_info->endianness : xenos::Endian::kNone;
    // Preserve the current native interpretation exactly. PrimitiveProcessor may
    // clamp the count, normalize endian, or convert indices; those draws retain
    // shader gathering. No memexport is possible at this host-RT-only seam.
    const uint32_t native_index_width = native_context.index_format == xenos::IndexFormat::kInt32 ? 4u : 2u;
    const uint64_t native_index_bytes = uint64_t(index_count) * native_index_width;
    const bool native_index_format_supported =
    ...
    native_context.hardware_index_eligible =
        index_buffer_info && native_context.indexed && native_index_format_supported &&
        ...
        primitive_processing_result.guest_draw_vertex_count == index_count &&
    ...
    native_context.use_hardware_indices =
        native_context.hardware_index_eligible && REXCVAR_GET(nb_native_hardware_indices);
    native_context.vertex_shader = vertex_shader;
    native_context.pixel_shader = pixel_shader;
    native_context.bound_render_target_bits = bound_depth_and_color_render_target_bits;
    for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i) {
      native_context.rtv_formats[i] =
          (bound_depth_and_color_render_target_bits & (uint32_t(1) << (1 + i)))
              ? render_target_cache_->GetColorDrawDXGIFormat(
                    xenos::ColorRenderTargetFormat(bound_depth_and_color_render_target_formats[1 + i]))
              : DXGI_FORMAT_UNKNOWN;
    }
    native_context.dsv_format =
        (bound_depth_and_color_render_target_bits & uint32_t(1))
            ? D3D12RenderTargetCache::GetDepthDSVDXGIFormat(
                  xenos::DepthRenderTargetFormat(bound_depth_and_color_render_target_formats[0]))
            : DXGI_FORMAT_UNKNOWN;
    // Same rule as PipelineCache::CreateD3D12Pipeline for the host render target path.
    const uint32_t guest_msaa_samples =
        uint32_t(1) << uint32_t(register_file_->Get<reg::RB_SURFACE_INFO>().msaa_samples);
    if (guest_msaa_samples == 2 && !render_target_cache_->msaa_2x_supported()) {
      native_context.sample_count = 4;
      native_context.sample_mask = 0b1001;
    } else {
      native_context.sample_count = guest_msaa_samples;
      native_context.sample_mask = UINT_MAX;
    }
    native_context.draw_resolution_scale_x = draw_resolution_scale_x;
    native_context.draw_resolution_scale_y = draw_resolution_scale_y;
    native_context.viewport_info = &viewport_info;
    // The old diagnostic path may request textures again inside the native
    // hook. Without another marker there, uploads could be misattributed to
    // geometry. Count that attempt as unclassified instead of guessing.
    const bool texture_request_deduplicated = !upload_order_scope.observation() ||
                                              REXCVAR_GET(nb_native_defer_guest_pipeline);
    bool native_recorded;
    {
      NativeUploadPhaseScope phase(upload_order_scope.observation(), NativeUploadPhase::kNative);
      nb::gpu::NativeRangeCallerScope origin(nb::gpu::NativeRangeCaller::kNativeHook,
                                             nb::gpu::NativeRangeDiagnosticsEnabled());
      NativeGpuPassScope native_gpu_region(*this, "native_hook_inclusive");
      native_recorded = TryNativeDraw(native_context);
    }
    upload_order_scope.CompleteNative(native_recorded, texture_request_deduplicated);
    if (native_recorded) {
      if (skip_guest_pipeline) ++native_guest_pipeline_skips_;
      return true;
    }
  }

  if (skip_guest_pipeline && !configure_guest_pipeline()) {
    return false;
  }
  // nb Phase 3: keep recording a pending emulated PSO handle. EndSubmission waits
  // for queued pipeline creation before the deferred list executes, so a null
  // resolved PSO at Execute means creation failed. A native success has no guest
  // pipeline to await and is not counted here. The fallback uses the same pending
  // handle and root-signature path regardless of when ConfigurePipeline ran.
  if (REXCVAR_GET(async_shader_compilation) &&
      pipeline_cache_->GetD3D12PipelineByHandle(pipeline_handle) == nullptr) {
    ++GetPhase3Counters().record_time_pipeline_skip_count;
  }

  // Texture loads and a refused native attempt may have changed the pipeline.
  // Successful native draws leave their external pipeline cached for reuse.
  if (defer_guest_pipeline && current_guest_pipeline_ != pipeline_handle) {
    deferred_command_list_.SetPipelineStateHandle(reinterpret_cast<void*>(pipeline_handle));
    current_guest_pipeline_ = pipeline_handle;
    current_external_pipeline_ = nullptr;
  ...
  NativeGpuPassScope emulated_gpu_region(*this, "emulated_draw_region", memexport_used);
  // Draw.
...
bool D3D12CommandProcessor::IssueDraw_MemexportReadbackFullPath(uint32_t total_size) {
  if (!FlushNativeReplay()) return false;
...
bool D3D12CommandProcessor::IssueCopy_ReadbackResolvePath() {
  // Restore the original fully submitted/completed lifetime before resizing or
  // reading the legacy readback slots, including a hot-reload into this mode.
  if (native_replay_worker_ && !AwaitAllQueueOperationsCompletion()) return false;
  uint32_t written_address, written_length;
...
bool D3D12CommandProcessor::CheckNativeReplayResult(HRESULT result) {
  if (SUCCEEDED(result)) return true;
  if (!device_removed_) {
    REXGPU_ERROR("Native asynchronous replay failed (0x{:08X}); submission completion is unproven",
                 static_cast<uint32_t>(result));
    device_removed_ = true;
    if (graphics_system_) graphics_system_->OnHostGpuLossFromAnyThread(false);
  }
  return false;
}

bool D3D12CommandProcessor::FlushNativeReplay() {
  return !native_replay_worker_ || CheckNativeReplayResult(native_replay_worker_->Wait());
}

std::array<uint64_t, 3> D3D12CommandProcessor::NativeReplayCounts() const {
  if (!native_replay_worker_) return {};
  const auto& counts = native_replay_worker_->stats();
  return {counts.submitted, counts.completed, counts.failed};
}

std::array<uint64_t, 5> D3D12CommandProcessor::NativeReplayChunkCounts() const {
  if (!native_replay_worker_) return {};
  const auto& counts = native_replay_worker_->stats();
  return {counts.chunk_sessions, counts.chunks_submitted, counts.chunks_completed,
          counts.chunk_sessions_completed, counts.chunks_failed};
}

bool D3D12CommandProcessor::EnsureNativeReplayWorker() {
  if (!native_replay_worker_attempted_) {
    native_replay_worker_attempted_ = true;
    native_replay_worker_ = nb::gpu::NativeReplayWorker::TryCreate(GetD3D12Provider().GetDevice(), *this);
    REXGPU_INFO("Native asynchronous replay worker {}",
                native_replay_worker_ ? "ready" : "unavailable; synchronous replay retained");
  }
  return native_replay_worker_ != nullptr;
}

bool D3D12CommandProcessor::EnsureWritableCommandAllocator() {
  if (command_allocator_writable_first_) return true;
  ID3D12CommandAllocator* allocator;
  if (FAILED(GetD3D12Provider().GetDevice()->CreateCommandAllocator(
          D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&allocator)))) {
    REXGPU_ERROR("Failed to create a command allocator");
    return false;
  }
  auto* entry = new (std::nothrow) CommandAllocator;
  if (!entry) {
    allocator->Release();
    return false;
  }
  command_allocator_writable_first_ = entry;
  command_allocator_writable_first_->command_allocator = allocator;
  command_allocator_writable_first_->last_usage_submission = 0;
  command_allocator_writable_first_->next = nullptr;
  command_allocator_writable_last_ = command_allocator_writable_first_;
  return true;
...
bool D3D12CommandProcessor::ReplayNativeDrawBoundary() {
  const int32_t threshold = REXCVAR_GET(nb_native_replay_chunk_draws);
  if (threshold <= 0 || !submission_open_ || !frame_open_ ||
      !REXCVAR_GET(nb_native_async_replay)) return true;
  if (++native_chunk_draws_ < static_cast<uint32_t>(threshold)) return true;
  // Bound optional readiness checks when the worker is busy or shaders pending.
  // A refusal retains the whole tail until another offer or EndSubmission.
  native_chunk_draws_ = 0;
  if (device_removed_) return false;
  if (scratch_buffer_used_ || active_occlusion_query_.valid || cache_clear_requested_ ||
      native_gpu_submission_timing_ || REXCVAR_GET(nb_native_submission_diagnostics) ||
      GetReadbackResolveMode(REXCVAR_GET(d3d12_readback_resolve)) != ReadbackResolveMode::kDisabled) return true;
  // Memexport keeps its existing readback behavior: FullPath finalizes and
  // awaits this submission before mapping; FastPath reads only an actually
  // completed earlier slot. CPU chunk completion never changes either fence.
  if (!EnsureNativeReplayWorker()) return true;
  if (!native_replay_worker_->ReadyForChunk()) {
    return CheckNativeReplayResult(native_replay_worker_->Result());
  }
  // This CP is the runtime producer. Queue dequeue and busy accounting share
  // one mutex, and PSO publication precedes decrementing busy. With neither
  // queued nor busy creation, null handles are failed rather than pending.
  // Retain the original EndSubmission wait/flush cadence for any remaining tail.
  if (pipeline_cache_->IsCreatingPipelines()) return true;
  if (!deferred_command_list_.ResolvePipelineHandlesForReplay()) {
    return CheckNativeReplayResult(E_INVALIDARG);
  }
  bool accepted;
  if (native_chunk_allocator_) {
    accepted = native_replay_worker_->TryAppendChunk(deferred_command_list_);
  } else {
    if (!EnsureWritableCommandAllocator()) return true;
    accepted = native_replay_worker_->BeginChunks(deferred_command_list_,
        command_allocator_writable_first_->command_allocator,
        GetD3D12Provider().GetDirectQueue(), submission_fence_, submission_current_);
    if (accepted) {
      native_chunk_allocator_ = command_allocator_writable_first_;
      command_allocator_writable_first_ = native_chunk_allocator_->next;
      native_chunk_allocator_->next = nullptr;
      if (!command_allocator_writable_first_) command_allocator_writable_last_ = nullptr;
    }
  }
  if (accepted) deferred_command_list_.Reset();
  return CheckNativeReplayResult(native_replay_worker_->Result());
}

[[noreturn]] void D3D12CommandProcessor::TerminateUnprovenNativeReplay() {
  // A failed signal cannot authorize destruction of raw resources referenced
  // by queued work. Join CPU replay, then terminate without GPU destructors.
  // Both native-subclass and backend teardown must pass this gate first.
  FlushNativeReplay();
  REXGPU_ERROR("Asynchronous replay shutdown could not prove GPU completion; terminating without resource teardown");
  std::fflush(nullptr);
  std::_Exit(EXIT_FAILURE);
}

void D3D12CommandProcessor::CheckSubmissionFence(uint64_t await_submission) {
  if (native_replay_worker_ && !CheckNativeReplayResult(native_replay_worker_->Result())) return;
  if (await_submission >= submission_current_) {
    ...
    if (queue_operations_done_since_submission_signal_) {
      ...
      if (SUCCEEDED(direct_queue->Signal(queue_operations_since_submission_fence_, fence_value)) &&
        ...
        if (wait_result == WAIT_OBJECT_0 && completed != UINT64_MAX && completed >= fence_value) {
          queue_operations_done_since_submission_signal_ = false;
        } else {
          REXGPU_ERROR("Out-of-submission queue operation completion was not proven");
        }
      } else {
        REXGPU_ERROR(
  ...
  const uint64_t observed_completion = submission_fence_->GetCompletedValue();
  if (observed_completion == UINT64_MAX) {
    CheckNativeReplayResult(DXGI_ERROR_DEVICE_REMOVED);
    return;
  }
  submission_completed_ = observed_completion;
  if (submission_completed_ < await_submission) {
    // A failed worker must never leave this thread waiting for a signal that
    // was not issued. Normal nonblocking frame checks retain CPU overlap.
    if (!FlushNativeReplay()) return;
    if (SUCCEEDED(
      ...
      if (completed_after_wait == UINT64_MAX) {
        CheckNativeReplayResult(DXGI_ERROR_DEVICE_REMOVED);
        return;
      }
      submission_completed_ = completed_after_wait;
    }
  }
  if (submission_completed_ < await_submission) {
...
bool D3D12CommandProcessor::BeginSubmission(bool is_guest_command) {
  ...
  if (native_gpu_budget_probe_) {
    native_gpu_budget_probe_->ObserveFrame(NativeGpuTimingFrame());
  }

  if ((native_replay_worker_ && !CheckNativeReplayResult(native_replay_worker_->Result())) || device_removed_) {
    return false;
  }
  ...
  if (!submission_open_) {
    ...
    assert_null(native_chunk_allocator_);
    native_chunk_draws_ = 0;
    ...
    deferred_command_list_.Reset();
    if (native_gpu_budget_probe_) {
      native_gpu_budget_probe_->BeginSubmission(deferred_command_list_, NativeGpuTimingFrame(),
          frame_current_, submission_current_);
      native_gpu_budget_scopes_active_ = native_gpu_budget_probe_->scopes_active();
      deferred_command_list_.SetNativeBudgetProbe(native_gpu_budget_probe_->recording_active()
          ? native_gpu_budget_probe_.get() : nullptr);
    }
    native_static_bindings_.InvalidateAll();
...
// nb: Phase 3 item 6. Writes one JSON line with the per-frame named counter
// set and resets the counters for the next frame. Called from EndSubmission
// on swap, on the command processor thread. Lazily opened; reopened if the
// cvar path changes. Never closed, same lifetime as the process.
static void NbDumpPhase3Counters(uint32_t frame) {
  ...
  if (open_path != path) {
    ...
    open_path = file != nullptr ? path : std::string();
    if (file == nullptr) {
      REXGPU_ERROR("nb_phase3_counters_file: failed to open '{}'", path);
      return;
    }
  }
  GetPhase3Counters().WriteJsonLine(file, frame);
  std::fflush(file);
  GetPhase3Counters().Reset();
}
...
bool D3D12CommandProcessor::EndSubmission(bool is_swap) {
  ...
  if (submission_open_ && !native_chunk_allocator_ && !EnsureWritableCommandAllocator()) return false;
  ...
  if (submission_open_) {
    ...
    // nb: Phase 3 item 6 / step 6. Time the pipeline creation await. With
    // the record-time draw drop removed (step 6), this wait is the residual
    ...
    if (native_gpu_budget_probe_) {
      const auto& counters = GetPhase3Counters();
      native_gpu_budget_probe_->EndSubmission(deferred_command_list_, is_closing_frame,
          counters.transfer_count, counters.transfer_tiles, counters.resolve_dump_count);
      native_gpu_budget_scopes_active_ = false;
    }
    ...
    CommandAllocator* submission_allocator = native_chunk_allocator_
        ? native_chunk_allocator_ : command_allocator_writable_first_;
    ID3D12CommandAllocator* command_allocator = submission_allocator->command_allocator;
    NativeSubmissionStats* submission_stats = REXCVAR_GET(nb_native_submission_diagnostics)
        ? &GetNativeSubmissionStats() : nullptr;
    const uint64_t native_timing_frame = native_gpu_submission_timing_ ? NativeGpuTimingFrame() : 0;
    bool queued_replay = false;
    const bool allow_replay = REXCVAR_GET(nb_native_async_replay) && !is_swap && frame_open_ &&
        !native_gpu_submission_timing_ && !submission_stats &&
        GetReadbackResolveMode(REXCVAR_GET(d3d12_readback_resolve)) == ReadbackResolveMode::kDisabled;
    if (native_chunk_allocator_) {
      // Once a prefix is consumed, the tail must use that same physical list,
      // even if an option changed or the original boundary is a final swap.
      if (!deferred_command_list_.ResolvePipelineHandlesForReplay() ||
          !native_replay_worker_->FinishChunks(deferred_command_list_)) {
        CheckNativeReplayResult(E_UNEXPECTED);
        return false;
      }
      queued_replay = true;
      if (is_swap && !FlushNativeReplay()) return false;
    } else if (allow_replay) {
      if (EnsureNativeReplayWorker()) {
        if (!deferred_command_list_.ResolvePipelineHandlesForReplay()) {
          CheckNativeReplayResult(E_INVALIDARG);
          return false;
        }
        // The allocator and stream are exclusively owned until this job ends.
        // The common bookkeeping below reserves this same fence identity before
        // the CP can observe completion or prepare another submission.
        queued_replay = native_replay_worker_->Submit(deferred_command_list_, command_allocator,
            direct_queue, submission_fence_, submission_current_);
      }
    }
    uint32_t native_timing_slot = nb::gpu::NativeGpuSubmissionTiming::kNoSlot;
    bool native_timing_close_valid = false;
    if (!queued_replay) {
      bool native_timing_reset_valid = true;
      {
        NativeSubmissionTimer timer(submission_stats ? &submission_stats->allocator_reset : nullptr);
        const HRESULT result = command_allocator->Reset();
        native_timing_reset_valid &= SUCCEEDED(result);
        timer.RecordFailure(FAILED(result));
      }
      {
        NativeSubmissionTimer timer(submission_stats ? &submission_stats->list_reset : nullptr);
        const HRESULT result = command_list_->Reset(command_allocator, nullptr);
        native_timing_reset_valid &= SUCCEEDED(result);
        timer.RecordFailure(FAILED(result));
      }
      if (native_gpu_submission_timing_ && !native_timing_reset_valid) {
        native_gpu_submission_timing_->FailedReset(native_timing_frame);
      }
      native_timing_slot = native_gpu_submission_timing_ && native_timing_reset_valid
          ? native_gpu_submission_timing_->Begin(command_list_, native_timing_frame, frame_current_,
              submission_current_, frame_open_, is_closing_frame)
          : nb::gpu::NativeGpuSubmissionTiming::kNoSlot;
      {
        NativeSubmissionTimer timer(submission_stats ? &submission_stats->replay : nullptr);
        deferred_command_list_.Execute(command_list_, command_list_1_);
      }
      if (native_gpu_submission_timing_) native_gpu_submission_timing_->End(command_list_, native_timing_slot);
      {
        NativeSubmissionTimer timer(submission_stats ? &submission_stats->close : nullptr);
        const HRESULT result = command_list_->Close();
        native_timing_close_valid = SUCCEEDED(result);
        timer.RecordFailure(FAILED(result));
      }
      ID3D12CommandList* execute_command_lists[] = {command_list_};
      if (native_gpu_submission_timing_) native_gpu_submission_timing_->BeforeExecute(native_timing_slot);
      {
        NativeSubmissionTimer timer(submission_stats ? &submission_stats->execute : nullptr);
        direct_queue->ExecuteCommandLists(1, execute_command_lists);
      }
      if (native_gpu_submission_timing_) native_gpu_submission_timing_->AfterExecute(native_timing_slot);
    }
    submission_allocator->last_usage_submission = submission_current_;
    if (native_chunk_allocator_) {
      native_chunk_allocator_ = nullptr;
    } else {
      command_allocator_writable_first_ = submission_allocator->next;
      if (!command_allocator_writable_first_) command_allocator_writable_last_ = nullptr;
    }
    if (command_allocator_submitted_last_) {
      command_allocator_submitted_last_->next = submission_allocator;
    } else {
      command_allocator_submitted_first_ = submission_allocator;
    }
    command_allocator_submitted_last_ = submission_allocator;
    command_allocator_submitted_last_->next = nullptr;

    if (!queued_replay) {
      const HRESULT native_timing_signal_result = direct_queue->Signal(submission_fence_, submission_current_);
      if (native_gpu_submission_timing_) native_gpu_submission_timing_->Submitted(
          native_timing_slot, native_timing_close_valid, SUCCEEDED(native_timing_signal_result));
      if (native_gpu_submission_timing_) native_gpu_submission_timing_->SubmittedPasses(
          submission_current_, native_timing_close_valid && SUCCEEDED(native_timing_signal_result));
    }
    ++submission_current_;

    if (native_gpu_budget_probe_ && !native_gpu_budget_probe_->finished() &&
        native_gpu_budget_probe_->capture_ended()) {
      const auto publication = native_gpu_budget_probe_->TryFinish(submission_fence_->GetCompletedValue());
      if (publication == nb::gpu::NativeGpuBudgetProbe::Publication::kWritten) {
        REXGPU_INFO("NB_GPU budget probe saved after captured submissions completed");
      } else if (publication == nb::gpu::NativeGpuBudgetProbe::Publication::kFailed) {
        REXGPU_WARN("NB_GPU budget probe invalid or could not be written");
      }
    }

    if (native_gpu_submission_timing_ && native_gpu_submission_timing_->NeedsCompletionCheck(native_timing_frame)) {
      const auto publication = native_gpu_submission_timing_->TryFinishAfterCapture(
          native_timing_frame, submission_fence_->GetCompletedValue());
      if (publication == nb::gpu::NativeGpuSubmissionTiming::Publication::kWritten) {
        REXGPU_INFO("Native GPU timestamp capture saved after captured submissions completed");
      } else if (publication == nb::gpu::NativeGpuSubmissionTiming::Publication::kFailed) {
        REXGPU_WARN("Native GPU timestamp capture incomplete or could not be written");
      }
    }
  ...
  // nb: Phase 3 item 6. Dump and reset the per-frame counters at the swap
  // boundary. counter() counts completed PM4_XE_SWAP packets
  // (command_processor.cpp:963), so it identifies the frame being closed.
  if (is_closing_frame) {
    NbDumpPhase3Counters(counter());
  }
...
bool D3D12CommandProcessor::UpdateBindings(const D3D12Shader* vertex_shader,
  ...
  // The emulated layout may overwrite any of the native geometry root slots.
  native_static_bindings_.InvalidateAll();
  const ui::d3d12::D3D12Provider& provider = GetD3D12Provider();
  ID3D12Device* device = provider.GetDevice();
  const RegisterFile& regs = *register_file_;
