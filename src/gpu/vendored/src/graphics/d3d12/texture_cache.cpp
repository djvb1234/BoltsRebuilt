...
REXCVAR_DEFINE_BOOL(nb_native_texture_descriptor_memo, false, "nb",
                    "Reuse the last successful SRV map lookup within each live texture; retain live binding and descriptor ownership")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

namespace nb::gpu {
namespace { NativeTextureDescriptorMemoStats texture_descriptor_memo_stats; }
const NativeTextureDescriptorMemoStats& GetNativeTextureDescriptorMemoStats() {
  return texture_descriptor_memo_stats;
}
}  // namespace nb::gpu
...
bool D3D12TextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture, bool load_base,
  ...
  // nb: a verified offline base upload writes this same SDK-owned texture.
  // Keep all mip/volume/resolve cases on the original path, and retain the
  // SDK's existing watch, SRV, sampler, state, and submission-lifetime handling.
  if (load_base && !load_mips && texture_key.base_page && !texture_key.mip_page &&
      !texture_key.mip_max_level && !texture_key.packed_mips && !texture_key.scaled_resolve &&
      !texture_key.signed_separate && !d3d12_texture.force_load_3d_tiling() &&
      ...
      (texture_key.format == xenos::TextureFormat::k_DXT1 ||
       texture_key.format == xenos::TextureFormat::k_DXT2_3 ||
       texture_key.format == xenos::TextureFormat::k_DXT4_5) &&
      !(texture_key.GetWidth() & 3) && !(texture_key.GetHeight() & 3)) {
    nb::gpu::OfflineTextureRequest request = {texture_key.GetWidth(), texture_key.GetHeight(),
        uint32_t(texture_key.format), uint32_t(texture_key.endianness), texture_key.tiled,
        uint32_t(texture_key.pitch) << 5, uint32_t(texture_key.base_page) << 12, texture.GetGuestBaseSize()};
    nb::gpu::OfflineTextureUpload upload;
    if (nb::gpu::PrepareOfflineTextureUpload(shared_memory(), command_processor_, request, upload)) {
      ...
      source.pResource = upload.buffer;
      source.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
      source.PlacedFootprint = upload.footprint;
      destination.pResource = d3d12_texture.resource();
...
uint32_t D3D12TextureCache::FindOrCreateTextureDescriptor(D3D12Texture& texture,
  ...
  auto* memo_stats = REXCVAR_GET(nb_native_texture_descriptor_memo)
                         ? &nb::gpu::texture_descriptor_memo_stats : nullptr;
  uint32_t existing_descriptor_index = texture.GetSRVDescriptorIndex(descriptor_key, memo_stats);
  ...
  texture.AddSRVDescriptorIndex(descriptor_key, descriptor_index, memo_stats);
  return descriptor_index;
}
