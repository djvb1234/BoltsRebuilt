...
namespace nb::gpu { class NativeGpuBudgetProbe; }
...
// Original nb diagnostics. Only the owning command-processor thread updates
// these counters; other threads receive their own independent totals.
struct NativeDeferredStorageStats {
  uint64_t enabled_resets = 0;
  uint64_t commands = 0;
...
class DeferredCommandList {
 public:
  ...
  DeferredCommandList(const DeferredCommandList&) = default;
  DeferredCommandList(DeferredCommandList&& other) noexcept;

  // One state per physical command-list recording. Reset it only when the
  // real list is reset, not between immutable chunks of that same list.
  struct ReplayState {
  ...
  void Execute(ID3D12GraphicsCommandList* command_list, ID3D12GraphicsCommandList1* command_list_1,
               ReplayState* replay_state = nullptr);
  size_t UsedBytes() const noexcept { return command_stream_used_ * sizeof(uintmax_t); }
  // CP-side, non-mutating optional-chunk check. A pending/failed handle or
  // malformed extent returns false without resolving commands or counters.
  // This does not replace final preflight after the original pipeline wait.
  bool IsReadyForReplay() const noexcept;
  // nb: CP-side preflight after PipelineCache::EndSubmission. Replace pending
  // handles with direct PSO commands so replay never consults the mutable CP.
  // Null handles keep the existing draw/dispatch suppression and CP counters.
  // False means a malformed owned stream; do not replay it on either thread.
  bool ResolvePipelineHandlesForReplay() noexcept;
  bool pipeline_handles_resolved_for_replay() const noexcept {
    return pipeline_handles_resolved_for_replay_;
  }
  // Both lists must have the same CP owner and be exclusively held by caller.
  // Exchanges all constructed storage and its logical extent without allocation.
  void SwapStorage(DeferredCommandList& other) noexcept;
  // Recording-side diagnostic pointer, deliberately NOT exchanged by SwapStorage.
  // Replay workers only execute immutable streams and never touch this object.
  void SetNativeBudgetProbe(nb::gpu::NativeGpuBudgetProbe* probe) { nb_budget_probe_ = probe; }
  ...
  D3D12_RECT* ClearDepthStencilViewAllocatedRects(D3D12_CPU_DESCRIPTOR_HANDLE depth_stencil_view,
    ...
    if (nb_budget_probe_) RecordNativeBudgetOperation(4, 0);
  ...
  void D3DCopyBufferRegion(ID3D12Resource* dst_buffer, UINT64 dst_offset,
    ...
    if (nb_budget_probe_) RecordNativeBudgetOperation(2, num_bytes);
  ...
  void D3DCopyResource(ID3D12Resource* dst_resource, ID3D12Resource* src_resource) {
    if (nb_budget_probe_) RecordNativeBudgetOperation(5, 0);
  ...
  void CopyTexture(const D3D12_TEXTURE_COPY_LOCATION& dst, const D3D12_TEXTURE_COPY_LOCATION& src) {
    if (nb_budget_probe_) RecordNativeBudgetOperation(3, 0);
  ...
  void D3DDispatch(UINT thread_group_count_x, UINT thread_group_count_y,
    ...
    if (nb_budget_probe_) RecordNativeBudgetOperation(1, 0);
  ...
  void D3DDrawIndexedInstanced(UINT index_count_per_instance, UINT instance_count,
    ...
    if (nb_budget_probe_) RecordNativeBudgetOperation(0, 0);
  ...
  // nb: Own the three native CBV addresses in one packet. Replay preserves the
  // separate API calls (root 0, root 1, then optional root 4), including a
  // requested null pixel view. The caller owns pixel-binding cache publication.
  void D3DSetNativeGraphicsConstantBufferViews(
      D3D12_GPU_VIRTUAL_ADDRESS root0, D3D12_GPU_VIRTUAL_ADDRESS vertex,
      D3D12_GPU_VIRTUAL_ADDRESS pixel, bool write_pixel) {
    auto& args = *reinterpret_cast<SetNativeGraphicsConstantBufferViewsArguments*>(
        WriteCommand(Command::kD3DSetNativeGraphicsConstantBufferViews,
                     sizeof(SetNativeGraphicsConstantBufferViewsArguments)));
    args.root0 = root0;
    args.vertex = vertex;
    args.pixel = pixel;
    args.write_pixel = write_pixel ? 1 : 0;
  }
 ...
 private:
#if defined(NB_NATIVE_DEFERRED_REPLAY_TEST)
  friend struct NativeDeferredReplayTestAccess;
#endif
  enum class Command {
  ...
  struct SetNativeGraphicsConstantBufferViewsArguments {
    D3D12_GPU_VIRTUAL_ADDRESS root0;
    D3D12_GPU_VIRTUAL_ADDRESS vertex;
    D3D12_GPU_VIRTUAL_ADDRESS pixel;
    // A full word keeps every payload byte initialized in retained storage.
    uint64_t write_pixel;
  };
  ...
  void RecordNativeBudgetOperation(uint32_t op, uint64_t copy_bytes);
  nb::gpu::NativeGpuBudgetProbe* nb_budget_probe_ = nullptr;
  ...
  // nb: size() is constructed high-water storage in the optional mode. Only
  // this prefix holds the current submission. No writes beyond vector size,
  // no borrowed caller payloads, and no stale trailing commands are replayed.
  size_t command_stream_used_ = 0;
  bool reuse_command_storage_ = false;  // selected only at Reset
  bool pipeline_handles_resolved_for_replay_ = false;  // CP preflight; invalidated by Reset/handle writes
};
