...
namespace nb::gpu { class NativeGpuSubmissionTiming; class NativeReplayWorker; class NativeGpuBudgetProbe; }

namespace rex::graphics::d3d12 {

class D3D12CommandProcessor : public CommandProcessor {
 public:
  // nb: the state TryNativeDraw receives (see the protected seam below); public so pass helper
  // classes outside the command processor can take it.
  struct NativeDrawContext {
    xenos::PrimitiveType guest_primitive_type;
    uint32_t index_count;
    bool indexed;
    // The guest index buffer when indexed (from IndexBufferInfo): byte address in guest memory, format and
    // endianness as the guest wrote it. A native pass reads it straight from shared memory.
    uint32_t index_guest_base;
    xenos::IndexFormat index_format;
    xenos::Endian index_endian;
    // True only for an unchanged direct guest DMA triangle list. Keep eligibility
    // separate from the diagnostic selection so native manual draws can be counted.
    bool hardware_index_eligible = false;
    bool use_hardware_indices = false;
    const D3D12Shader* vertex_shader;
    const D3D12Shader* pixel_shader;  // null when the pixel shader is disabled
    // GetLastUpdateBoundRenderTargets: bit 0 = depth, bits 1..4 = color targets 0..3.
    uint32_t bound_render_target_bits;
    DXGI_FORMAT rtv_formats[xenos::kMaxColorRenderTargets];  // DXGI_FORMAT_UNKNOWN when unbound
    DXGI_FORMAT dsv_format;                                  // DXGI_FORMAT_UNKNOWN when unbound
    // What a graphics pipeline drawing into these targets must use.
  ...
  // nb: guest memory as the GPU sees it (the 512 MB shared-memory buffer, raw SRV), for native
  // passes that read the guest's vertex data; call RequestRange and UseForReading before drawing.
  D3D12SharedMemory& shared_memory() { return *shared_memory_; }
  ...
  DeferredCommandList& GetDeferredCommandList() {
    ...
    // A raw-list caller may write graphics roots directly. Only the geometry
    // helpers below promise to preserve the tracked native root bindings.
  ...
  // nb: whether a guest occlusion query is open on the deferred list, so diagnostic host occlusion queries
  // (the UW15 stencil verification) can stay out of it rather than nest in it or inflate its count.
  bool IsGuestOcclusionQueryActive() const { return active_occlusion_query_.valid; }

  // Drain earlier CPU replay before a direct queue operation. An unfinished
  // chunk session remains open and unsubmitted. This is not GPU completion.
  bool FlushNativeReplay();
  std::array<uint64_t, 3> NativeReplayCounts() const;
  std::array<uint64_t, 5> NativeReplayChunkCounts() const;
  // Called only after IssueDraw has returned, with no borrowed write pointers.
  // Optional handoff rotates stream storage, never GPU submission/state caches.
  bool ReplayNativeDrawBoundary();
  ...
  // nb: resolve a whole native draw's samplers together. An overflow retries the entire batch on
  // the new heap, so the returned indices all belong to GetSamplerBindlessHeapGPUStart(). Consume
  // them before another sampler request; never cache them across draws.
  bool RequestSamplerBindlessIndices(const D3D12TextureCache::SamplerParameters* parameters,
                                    uint32_t count, uint32_t* indices_out);
  D3D12_GPU_DESCRIPTOR_HANDLE GetSamplerBindlessHeapGPUStart() const {
    assert_true(bindless_resources_used_);
    return sampler_bindless_heap_gpu_start_;
  }
  ...
  const ShaderLoadMemo::Stats& native_shader_load_memo_stats() const {
    return pipeline_cache_->shader_load_memo_stats();
  }
  ...
  // Geometry-only recording contract: sets the signature and roots 2/5/3/6,
  // then returns a list on which the caller may write dynamic roots 0/1,
  // bind an IA index buffer and draw. Root 4 must use SetNativePixelConstantBuffer
  // or the root-CBV-only SetNativeGeometryConstantBuffers compound form.
  // Do not retain the list across another pass or write tracked roots through
  // it. Other external users must request the ordinary raw list.
  DeferredCommandList& SetNativeGeometryStaticBindings(
  ...
  // Call after SetNativeGeometryStaticBindings, with an immutable allocation
  // kept alive until the submission completes. Publishes the address only after
  // appending a root-4 write; disabled mode writes and clears pixel validity.
  void SetNativePixelConstantBuffer(D3D12_GPU_VIRTUAL_ADDRESS address);
  // Same root0/root1/optional-root4 setters in one owned deferred packet. The
  // caller has selected the b0-CBV signature and keeps every allocation alive.
  // Pixel memo publication follows successful append, exactly as above.
  void SetNativeGeometryConstantBuffers(D3D12_GPU_VIRTUAL_ADDRESS root0,
                                       D3D12_GPU_VIRTUAL_ADDRESS vertex,
                                       D3D12_GPU_VIRTUAL_ADDRESS pixel);
  struct NativeConstantBindingPacketStats {
    uint64_t packets = 0, pixel_writes = 0;
  };
  const NativeConstantBindingPacketStats& native_constant_binding_packet_stats() const {
    return native_constant_binding_packet_stats_;
  }
  const NativeStaticBindings::Stats& native_static_binding_stats() const {
    return native_static_bindings_.stats();
  }
  uint64_t native_guest_pipeline_skips() const { return native_guest_pipeline_skips_; }
  ...
  uint32_t BeginNativeGpuPass(const char* name, uint64_t detail0 = 0,
                             uint64_t detail1 = 0, uint64_t detail2 = 0);
  void EndNativeGpuPass(uint32_t slot);
  uint32_t BeginNativeBudgetScope(const char* name, uint64_t detail0);
  void EndNativeBudgetScope(uint32_t previous);
  class NativeGpuPassScope {
   public:
    NativeGpuPassScope(D3D12CommandProcessor& cp, const char* name,
                      uint64_t a = 0, uint64_t b = 0, uint64_t c = 0)
        : cp_(cp), slot_(cp.BeginNativeGpuPass(name, a, b, c)),
          budget_previous_(cp.native_gpu_budget_scopes_active_
              ? cp.BeginNativeBudgetScope(name, a) : UINT32_MAX) {}
    ~NativeGpuPassScope() {
      if (slot_ != UINT32_MAX) cp_.EndNativeGpuPass(slot_);
      if (budget_previous_ != UINT32_MAX) cp_.EndNativeBudgetScope(budget_previous_);
    }
    NativeGpuPassScope(const NativeGpuPassScope&) = delete;
    NativeGpuPassScope& operator=(const NativeGpuPassScope&) = delete;
   private:
    D3D12CommandProcessor& cp_;
    uint32_t slot_;
    uint32_t budget_previous_;
  };

 protected:
  // Native subclasses can align captures with their swap-only frame logs. The
  // SDK guest counter also advances on timer vblanks and is unsuitable here.
  virtual uint64_t NativeGpuTimingFrame() const { return frame_current_; }
  // Native extensions retire their GPU resources before SharedMemory is destroyed.
  bool AwaitNativeResourcesIdle() {
    // A failed EndSubmission can leave queued draws open even when every older
    // fence completed. They must not reference resources the extension frees.
    const bool idle = AwaitAllQueueOperationsCompletion() && !submission_open_ &&
                      !queue_operations_done_since_submission_signal_;
    if (!idle && native_replay_worker_) TerminateUnprovenNativeReplay();
    return idle;
  }
  // nb: native-draw seam (src/gpu/vendored/UPSTREAM.md). IssueDraw calls TryNativeDraw once the
  // render targets, viewport, scissor, blend factor and stencil reference of the guest draw are
  // bound to the deferred command list and before the emulated draw is recorded. A subclass that
  // records its own draw instead (SetExternalGraphicsRootSignature, SetExternalPipeline,
  // SetPrimitiveTopology, GetDeferredCommandList(), SubmitBarriers()) returns true and the guest
  // draw is skipped; those helpers keep the cached pipeline state consistent, so the next guest
  // draw rebinds its own pipeline. Memexport draws are never offered.
  virtual bool TryNativeDraw(const NativeDrawContext& context) {
    (void)context;
    return false;
  }
  // nb: display-aspect seam (src/gpu/vendored/UPSTREAM.md). IssueSwap hands the presenter the guest
  // video mode's display size as the aspect to fit the guest output into. A subclass may replace it;
  // nb_ultrawide passes 32:9, so the anamorphic 1280x720 frame fills a 32:9 screen at 1:1. It is called
  // once per swap on the command-processor thread.
  virtual void OverrideDisplayAspect(uint32_t& display_width, uint32_t& display_height) {
    (void)display_width;
    (void)display_height;
  }
  // nb: what a native pass needs to sample the guest textures the replaced draw would have sampled.
  // RequestTextures has already run for the draw when TryNativeDraw is called, so the bindless SRV
  // indices are valid for the current bindings.
  D3D12TextureCache& texture_cache() { return *texture_cache_; }
 ...
 private:
  ...
  bool AwaitAllQueueOperationsCompletion() {
    ...
    return FlushNativeReplay() && !device_removed_ &&
           !submission_open_ && !native_chunk_allocator_ &&
           !queue_operations_done_since_submission_signal_ &&
           submission_completed_ != UINT64_MAX &&
           submission_completed_ + 1 >= submission_current_;
  }
  void LogDeviceRemovalDiagnostics(ID3D12Device* device, HRESULT reason);
  bool CheckNativeReplayResult(HRESULT result);
  bool EnsureNativeReplayWorker();
  bool EnsureWritableCommandAllocator();
  [[noreturn]] void TerminateUnprovenNativeReplay();
  std::unique_ptr<nb::gpu::NativeReplayWorker> native_replay_worker_;
  bool native_replay_worker_attempted_ = false;
  ...
  // Detached from the writable pool at the first accepted chunk, and retained
  // until the same original submission is finalized. Never reclaimed by a
  // completed CPU chunk or a fence belonging to an earlier submission.
  CommandAllocator* native_chunk_allocator_ = nullptr;
  uint32_t native_chunk_draws_ = 0;
  ...
  // Shared by the emulated and native sampler requests, including submission-safe heap retirement.
  bool RotateSamplerBindlessHeap();
  ...
  // Appended so the offsets of existing SDK fields do not change.
  NativeStaticBindings native_static_bindings_;
  uint64_t native_guest_pipeline_skips_ = 0;
  // Bounded diagnostic capture, owned until the existing queue drain at shutdown.
  std::unique_ptr<nb::gpu::NativeGpuSubmissionTiming> native_gpu_submission_timing_;
  // Independent probe: unlike legacy timing, this does not disable async replay.
  std::unique_ptr<nb::gpu::NativeGpuBudgetProbe> native_gpu_budget_probe_;
  bool native_gpu_budget_scopes_active_ = false;
  NativeConstantBindingPacketStats native_constant_binding_packet_stats_;
};
