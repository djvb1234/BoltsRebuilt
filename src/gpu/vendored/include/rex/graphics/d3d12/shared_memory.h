...
namespace nb::gpu { class NativeUploadShadow; }
...
class D3D12SharedMemory : public SharedMemory {
 ...
 private:
  ...
  // nb: byte identities belong to this exact shared-memory resource lifetime.
  bool UpdateUploadShadow(bool enabled);
  bool UploadRangesWithShadow(
      const std::vector<std::pair<uint32_t, uint32_t>>& upload_page_ranges);
  static void UploadShadowInvalidated(
      const std::unique_lock<std::recursive_mutex>& global_lock, void* context,
      uint32_t address_first, uint32_t address_last, bool invalidated_by_gpu);
  std::unique_ptr<nb::gpu::NativeUploadShadow> upload_shadow_;
  std::unique_ptr<uint8_t[]> upload_shadow_scratch_;
  GlobalWatchHandle upload_shadow_watch_ = nullptr;
  bool upload_shadow_requested_ = false, upload_shadow_failed_ = false;
  bool upload_shadow_retrying_ = false;
