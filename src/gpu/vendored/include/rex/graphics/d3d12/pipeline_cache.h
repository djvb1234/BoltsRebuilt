...
#include <rex/graphics/d3d12/shader_load_memo.h>
...
class PipelineCache {
 public:
  ...
  const ShaderLoadMemo::Stats& shader_load_memo_stats() const { return shader_load_memo_.stats(); }
  ...
  // Prepare the exact shader modifications synchronously, including binding
  // layouts and persistent shader storage. Does not select or create a PSO.
  // The command processor thread must call this before reading texture bindings.
  bool PrepareShaderTranslations(D3D12Shader::D3D12Translation* vertex_shader,
 ...
 private:
  ...
  // Processor-thread-only lookup accelerator. Ordinary CP ClearCaches retains
  // shaders; Shutdown and shader-storage reload reset this before deletion.
  ShaderLoadMemo shader_load_memo_;
};
